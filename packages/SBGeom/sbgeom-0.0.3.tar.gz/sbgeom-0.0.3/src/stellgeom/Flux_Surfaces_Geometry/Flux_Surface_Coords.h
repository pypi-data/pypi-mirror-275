...
struct Flux_Surface_Settings{
    unsigned number_of_surfaces = 0; /**<  Number of flux surfaces contained in the .dat files */
    unsigned n_tor              = 0; /**<  Number of toroidal harmonics present*/
    unsigned m_pol              = 0; /**<  Number of poloidal harmonics present*/
    unsigned symmetry           = 1; /**<  Symmetry of the reactor.*/
};
...
struct Radial_Flux_Coordinate{
    public: 
        ...
        Radial_Flux_Coordinate(double s, double distance_lcfs) : m_s(s), m_distance_lcfs(distance_lcfs) {if(s < 1.0 && m_distance_lcfs > 1e-5){
            throw std::invalid_argument("Trying to have a point inside the plasma ( s= "+std::to_string(s)+"<1.0" + " with a finite distance from the LCFS ("+std::to_string(distance_lcfs) +")");};//Check whether distance is zero if < lcfs
