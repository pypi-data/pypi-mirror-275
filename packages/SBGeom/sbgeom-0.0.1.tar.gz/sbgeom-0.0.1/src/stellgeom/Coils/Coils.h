...
class Coil{
    ...
    */
    ...
    Vector& Set_Centre()      { return m_centre;}; 
...
class Discrete_Coil : public Coil{
    public: 
        ...
        VectorArray& Set_Vertices()                       {return m_vertices;};
...
class Fourier_Coil : public Coil{
    public:
        ...
        VectorArray Return_Sampling_Curve() const override{
            ...
            VectorArray result(2 * nf, 3);
            for(unsigned i =0; i < 2 * nf; ++i){
                auto pos = this->Position(double(i) / double(2 * nf));
                result(i,0) = pos[0];
                result(i,1) = pos[1];
                result(i,2) = pos[2];
            }
        ...
        VectorArray& Set_Fourier_Cos()       {return m_xyz_f_cos;}

        VectorArray  Get_Fourier_Sin() const {return m_xyz_f_sin;}
        VectorArray& Set_Fourier_Sin()       {return m_xyz_f_sin;}
