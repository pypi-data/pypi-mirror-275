...
//        static std::unique_ptr<Flux_Surfaces> Construct_Flux_Surface(const json& json);
         ...
         * @return Unit_Vector 
         */
        Unit_Vector Return_Surface_Normal(const Flux_Surface_Coordinates& flux_surface_coordinates) const;
...
/**
 * @brief Creates an .stl file of 1 or 2 flux surfaces (connected if 2) for a given Toroidal_Extent
 * 
 * @param flux_surface 
 * @param r 
 * @param N_u 
 ...
 * @param toroidal_extent 
 */
void To_STL_Flux_Surface(const Flux_Surfaces& flux_surface, std::vector<Radial_Flux_Coordinate> r, unsigned N_u, unsigned N_v, const Toroidal_Extent& toroidal_extent);
