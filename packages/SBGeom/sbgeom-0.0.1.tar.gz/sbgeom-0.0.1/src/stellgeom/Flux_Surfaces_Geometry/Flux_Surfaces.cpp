...
Unit_Vector Flux_Surfaces::Return_Surface_Normal(const Flux_Surface_Coordinates& flux_surface_coordinates) const{
    ...
    Vector axis_to_position = Vector(x,y,z) - this->Return_Axis_Position(v);
...
void To_STL_Flux_Surface(const Flux_Surfaces& flux_surface, std::vector<Radial_Flux_Coordinate> r_vector, unsigned N_u, unsigned N_v, const Toroidal_Extent& toroidal_extent){
    if(r_vector.size() > 2){
        throw std::invalid_argument("Cannot create a .stl file from more than 2 flux surfaces");
    }
    ...
    if(r_vector.size() == 1){
        std::cout<<"Creating a .stl from 1 flux surface, i.e., from the flux surface at s="<<r_vector[0].Get_s()<<", d="<<r_vector[0].Get_distance_LCFS()<<std::endl;
        includes_axis = true;
        r_max = r_vector[0];
    }
    else{  
    ...
    if(d_s  < 0.0 || d_d < 0.0){throw std::invalid_argument("Wrongly ordered Flux Surfaces for To_STL_Flux_Surface.... Aborting...");}
    ...
    auto uv_manifold_vector = std::vector<UV_Manifold>();

    if(! includes_axis){
        uv_manifold_vector.push_back(flux_surface.Return_UV_Manifold(r_in_between[0],N_u,N_v, toroidal_extent));
    }

    uv_manifold_vector.push_back(flux_surface.Return_UV_Manifold(r_in_between.back(), N_u,N_v, toroidal_extent));

    auto triangle_vector = std::vector<Triangle_Vertices>();
    for(auto& i : uv_manifold_vector){
        triangle_vector.push_back(i.Mesh_Surface());
    }
    ...
    std::string filename = "UV_FLUX";
    output_file.open(filename+"_rmin_"+std::to_string(r_min.Get_distance_LCFS())+"_rmax_"+std::to_string(r_max.Get_distance_LCFS())+".stl");
    output_file << "solid Flux_Surface\n";

    if(triangle_vector.size() == 1){
        Append_To_STL(triangle_vector[0], output_file);
    }
    else{
        Append_To_STL(triangle_vector[0], output_file, true);
        Append_To_STL(triangle_vector[1], output_file);
    }
    ...
    if(include_edges){
        ...
        Append_To_STL(mesh_plane(false, 0), output_file);
        Append_To_STL(mesh_plane(true, N_v - 1), output_file);
    }

    output_file << "endsolid Flux_Surface\n";
	output_file.close();
