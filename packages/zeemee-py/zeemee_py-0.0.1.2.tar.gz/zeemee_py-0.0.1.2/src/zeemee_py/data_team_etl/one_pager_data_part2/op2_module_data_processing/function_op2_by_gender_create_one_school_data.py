...
def complie_op2_dataframe_by_gender_for_given_school(cohort_year, entry_term, student_type, zeemee_college_file_path, BASE_PATH, table_name):
     import json
     import os
     import pandas as pd
     import sys
     ...
     if not all(item in accepted_gender_values for item in gender_values_in_data):
          ...
          for i in gender_values_in_data:
               ...
               op_for_current_gender_df = pd.DataFrame()
               op_for_current_gender_df['org_id'] = organization_id
               op_for_current_gender_df['cohort_year'] = [cohort_year]
               op_for_current_gender_df['entry_term'] = [entry_term]
               op_for_current_gender_df['student_type'] = [student_type]
               op_for_current_gender_df['gender'] = [current_gender_value]

               sys.path.insert(0,BASE_PATH + '/one_pager_data_part_2/op2_module_data_processing/')
               import function_common_op2_calculate_values
               importlib.reload(function_common_op2_calculate_values)
               current_file_values_df = function_common_op2_calculate_values.get_op2_values(BASE_PATH, filtered_df)
               
               op_for_current_gender_df = pd.concat([op_for_current_gender_df, current_file_values_df], axis=1)
               final_df_for_org_with_all_genders = pd.concat([final_df_for_org_with_all_genders, op_for_current_gender_df])
...
if __name__ == "__main__":
     ...
     zeemee_college_file_path = '/home/luis/Zemee College_data_project/data-team/data_team_etl/data_store/zeemee_college_combined_files/Augsburg University_zeemee_college_combined.csv'
     BASE_PATH = "/home/luis/Zemee College_data_project/data-team/data_team_etl"
     table_name = 'one_pager_data_part2_by_gender'
     file = complie_op2_dataframe_by_gender_for_given_school(cohort_year, entry_term, student_type, zeemee_college_file_path, BASE_PATH, table_name)
     file.to_csv("/home/luis/Zemee College_data_project/data-team/data_team_etl/one_pager_data_part2/op2_module_data_processing/test_data.csv", index = False)
