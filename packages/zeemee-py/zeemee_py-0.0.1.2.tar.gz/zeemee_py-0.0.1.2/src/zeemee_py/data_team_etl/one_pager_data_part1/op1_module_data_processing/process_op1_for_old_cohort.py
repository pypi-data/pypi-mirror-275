...
def process_for_zeemee_college_combined_files(BASE_PATH, table_name, cohort_year, PATH_TO_OLD_COHORT_COMBINED_FILES):
     ...
     import get_zeemee_college_combined_files_details_for_old_cohort
     importlib.reload(get_zeemee_college_combined_files_details_for_old_cohort)
     combined_files_info = get_zeemee_college_combined_files_details_for_old_cohort.get_files_in_combined_files_folder(PATH_TO_OLD_COHORT_COMBINED_FILES)
     
     #drop rows that have null values and record those to list for logging 
     is_null_in_rows = combined_files_info.isnull().any(axis=1)
     ...
     for i in range(len(is_null_in_rows)):
          if is_null_in_rows[i] == True:
               organizations_not_processed.append(combined_files_info.loc[i, 'org_name'])
               not_processed_reason.append('null value in combined_files_info_files_info')
     ...
     combined_files_info = combined_files_info.dropna().reset_index(drop =True)
     
     sys.path.insert(0,BASE_PATH + '/one_pager_data_part1/op1_module_data_processing/')
     import function_op1_create_one_school_data
     importlib.reload(function_op1_create_one_school_data)
     ...
     for i in entry_term_list:
          for j in student_type_list:
               ...
               for school_no in range(len(combined_files_info)):
                    school_name = combined_files_info.loc[school_no, 'org_name']
                    org_id = combined_files_info.loc[school_no, 'organization_id']
                    combined_file_path = combined_files_info.loc[school_no, 'combined_file_path']
                    
                    print("\tRunning for " + school_name + ' ' + org_id + ' ' + str(current_cohort_year) + ' ' + current_entry_term + ' ' + current_student_type.replace(' ', '_'))
                    
                    try:
                         current_school_df  = function_op1_create_one_school_data.complie_op1_dataframe_for_given_school(current_cohort_year, current_entry_term, current_student_type, combined_file_path, BASE_PATH, table_name)
                         ...
                         print("\terror in function_op1_create_one_school_data for " + school_name + ' ' + str(current_cohort_year) + current_entry_term + current_student_type.replace(' ', '_') + ' error:' + str(e))
                         organizations_not_processed.append(school_name)
                         not_processed_reason.append('error in function_op1_create_one_school_data')
...
if __name__ == '__main__':
     ...
     combined_school_df_file_path_list = process_for_zeemee_college_combined_files(BASE_PATH, table_name, cohort_year, PATH_TO_OLD_COHORT_COMBINED_FILES)
