...
def stitch_function_for_op2_by_gender_for_old_cohort():
     ...
     print("Starting main stitch for op2 by gender for config schools")
     sys.path.insert(0,BASE_PATH + '/one_pager_data_part2/op2_module_data_processing/')
     import process_op2_by_gender_for_old_cohort
     importlib.reload(process_op2_by_gender_for_old_cohort)
     combined_school_df_file_path = process_op2_by_gender_for_old_cohort.process_by_gender_for_zeemee_college_combined_files(BASE_PATH, table_name, cohort_year, PATH_TO_OLD_COHORT_COMBINED_FILES)
     ...
     for i in range(len(combined_school_df_file_path)):
          ...
          import function_op2_perform_athena_update
          importlib.reload(function_op2_perform_athena_update)
          function_op2_perform_athena_update.concat_and_update_athena_table(BASE_PATH, table_name, combined_school_df_file_path_i)
          
     end_time = datetime.now()
     dt_string = end_time.strftime("%Y-%m-%d %H:%M:%S")
     print("op2 by gender for old_cohort {cohort_year} complete. \nEnd time ={dt_string}".format(cohort_year = str(cohort_year), dt_string = dt_string))
     print("Total time for processing:", end_time - start_time)

if __name__ == '__main__':
     stitch_function_for_op2_by_gender_for_old_cohort()
