...
def get_latest_files():
     ...
     post_wrangler_file_path_in_s3 = list()
     for files in bucket.objects.filter(Prefix="zeemee_py/latest_postwranger_files/").all():
         post_wrangler_file_path_in_s3.append("s3://" + datateams3bucket + "/" + files.key)
     
     post_wrangler_file_path_in_s3 = list(sorted(post_wrangler_file_path_in_s3))
     org_id_of_file = list()
     s3_file_path = list()
     
     for i in post_wrangler_file_path_in_s3:
          try:
     ...
     latest_postwrangler_files_info = pd.DataFrame()
     latest_postwrangler_files_info["organization_id"] = org_id_of_file
     latest_postwrangler_files_info["filepath_in_s3"] = s3_file_path
     
     latest_postwrangler_files_info = partner_df.merge(
          latest_postwrangler_files_info,
          on = "organization_id",
          how = "left"
          )
     
     return latest_postwrangler_files_info
