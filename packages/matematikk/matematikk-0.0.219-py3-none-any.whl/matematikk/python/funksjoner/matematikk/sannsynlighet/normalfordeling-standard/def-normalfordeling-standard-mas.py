...
def sannsynlighet_x_fra_mu_sd(
    ...
    # Hvis svaret konverteres til prosent
    if is_prosent   != -1:
        _prosent_sym = " %"
        sannsyn *= 100

    # Hvis svaret avrundes
    ...
    if is_print     != -1:
        ...
        print(f"")
        if print_txt != str():
            print(f"{print_txt}{sannsyn}{_prosent_sym}")

        if print_txt == str():
            print(f"Sannsynligheten for x {_x_ulikhetstegn} {_gunstig} er: {sannsyn}{_prosent_sym}")
