...
def create_resource(attributes=None, resource_detectors=None):
    ...
    """
    resource_detectors = DEFAULT_DETECTORS + (resource_detectors if resource_detectors else [])
...
def autoinstrument_app(app, instrumentors=None):
    """Autoinstrument with installed instrumentors.

    You can give a list of instrumentors to load.
    """
...
def new_span(name, unwrap_attributes_from_return_value=False, return_value_as=None, args_as=None):
                    ...
                    for idx, arg_name in enumerate(sig.parameters):
                        ...
                        if arg_name in args_as:
                            if param.kind is Parameter.POSITIONAL_ONLY:
                                arg_value = args[idx] if idx < len(args) else param.default
                            if param.kind is Parameter.POSITIONAL_OR_KEYWORD:
                                arg_value = kwargs.get(
                                    arg_name, args[idx] if idx < len(args) else param.default
                                )
...
class FlaskConfigResourceDetector(ResourceDetector):
    """Flask config resource detector.

    Small class to subclass from to create an app-specific ResourceDetector, by
    overriding query() and returning additional attributes.
    """
    ...
    def query(self):
        ...
        Return a dictionary with attributes to add to the resource.
        """
        return {"flask.debug": self._app.config.get("DEBUG", None)}

    def detect(self):
        """Detect the attributes."""
        if self._app:
            return Resource(self.query())
        return Resource.get_empty()


DEFAULT_DETECTORS = [PlatformResourceDetector()]
