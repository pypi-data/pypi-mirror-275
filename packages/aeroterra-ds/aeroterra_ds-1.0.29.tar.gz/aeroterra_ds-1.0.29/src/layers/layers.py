...
def update_layer(gdf, gis, layer_id, columns=None):
    ...
    total = []
    for i in range(0, len(feature_set), 1000):
        end = i + 1000
...
def read_full_layer_aux_sdf(layer, out_crs=None):
    ...
    new_data = layer.query(out_sr=out_crs, result_record_count=10000, return_all_records=False).sdf
    while len(new_data) > 0:
        ...
        new_data = layer.query(where = new_where, out_sr=out_crs, result_record_count=10000, return_all_records=False).sdf
...
def read_layer_gdf(gis, layer_id, number=None, query=None, geometry_filter=None, geometry_crs=4326, out_fields=None, out_crs=None):
    ...
    total_data = None
    new_data = layer.query(where = basic_where, geometry_filter=geo_filter, out_fields=out_fields, out_sr=out_crs, result_record_count=10000, return_all_records=False).sdf
    while len(new_data) > 0:
        if total_data is None:
            total_data = new_data.copy()
        else:
        ...
        if query:
            ...
            new_where = f"{object_id_col} > {last_time}"
        new_data = layer.query(where = new_where, geometry_filter=geo_filter, out_fields=out_fields, out_sr=out_crs, result_record_count=10000, return_all_records=False).sdf
