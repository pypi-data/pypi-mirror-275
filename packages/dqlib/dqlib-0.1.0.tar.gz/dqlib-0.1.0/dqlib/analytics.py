...
def create_ir_curve_risk_settings(delta=False,
    ...
    '''
    return dqCreateProtoIrCurveRiskSettings(delta,
                                            gamma,
                                            curvature,
                                            shift,
                                            curvature_shift,
...
def create_credit_curve_risk_settings(delta=False,
    ...
    '''
    return dqCreateProtoCreditCurveRiskSettings(delta,
                                                gamma,
                                                shift,
                                                method,
                                                granularity,
...
# Theta Risk Settings:
def create_theta_risk_settings(theta=False,
                               shift=1,
                               scaling_factor=1. / 365.):
    '''
    创建计算Theta的参数设置.
    ...
    '''
    return dqCreateProtoThetaRiskSettings(theta,
                                          shift,
                                          scaling_factor)
...
def create_credit_curve(as_of_date,
    ...
    cs_curve = dqCreateProtoCreditCurve(p_ts_curve)
    return cs_curve
...
def print_term_structure_curve(curve):
    ...
    for i in range(len(curve.pillar_date)):
        ...
        value = curve.curve.interpolator.ordinates.data[i]
...
def create_price_risk_settings(delta=False,
                               ...
                               curvature=False,
                               shift=1.0e-2,
                               curvature_shift=5.0e-1,
                               method=CENTRAL_DIFFERENCE_METHOD,
                               scaling_factor=1.0e-2,
                               threading_mode=SINGLE_THREADING_MODE):
    """
    ...
    shift : float, optional
        The shift size for the curve delta and gamma calculation. The default is 1.0e-2.
    curvature_shift : float, optional
        The shift size for the curvature calculation. The default is 5.0e-1.
    method : FiniteDifferenceMethod, optional
        The finite difference method for the curve delta calculation. The default is 'CENTRAL_DIFFERENCE_METHOD'.
    scaling_factor : float, optional
        The scaling factor for calculating USD amount sensitivity. The default is 1.0e-2.
    threading_mode : ThreadingMode, optional
...
def create_price_vol_risk_settings(vanna=False,
                                   ...
                                   method=CENTRAL_DIFFERENCE_METHOD,
                                   granularity=TOTAL_RISK,
                                   price_scaling_factor=1.0e-2,
                                   vol_scaling_factor=1.0e-2,
                                   threading_mode=SINGLE_THREADING_MODE):
    """
...
def create_dividend_curve_risk_settings(delta=False,
    ...
    :param gamma: bool
    :param shift: float
    :param method: FiniteDifferenceMethod
    :param granularity: RiskGranularity
    :param scaling_factor: float
    :param threading_mode: ThreadingMode
    :return: DividendCurveRiskSettings
    """
    return dqCreateProtoDividendCurveRiskSettings(delta,
                                                  gamma,
                                                  shift,
                                                  method,
                                                  granularity,
...
def create_inst_price(currency,
                      amount):
    pass
