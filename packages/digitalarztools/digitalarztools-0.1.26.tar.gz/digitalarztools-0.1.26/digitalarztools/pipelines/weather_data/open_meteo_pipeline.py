...
class OpenMeteoPipeline:
    ...
    @staticmethod
    def calculate_stats_from_hourly_data(hourly_df: pd.DataFrame) -> pd.DataFrame:
        ...
        daily_summary = hourly_df.groupby('day').agg({
            'rain': 'sum',
            'snowfall': 'sum',
            'snow_depth': 'mean',
            'precipitation': 'sum',
        ...
        daily_summary.columns = ['_'.join(col).strip() if isinstance(col, tuple) else col for col in
