...
class CompoundMessage(GenericCRUD, metaclass=MetaLogger, object=object_message):
    ...
    def get_recipients_by_criteria_set_ids(self, criteria_set_ids: list[int]) -> list[Recipient]:
        ...
        qeury = f"""
            ...
            WHERE profile_ml.profile_id IN ({", ".join(["%s"] * len(all_profile_ids))});
        """
        ...
        for profile_id, profile_dicts in profile_dicts.items():
            ...
            name_per_lang_code_str = {row["lang_code_str"]: row["title"] for row in profile_dicts}
            recipients.append(Recipient(profile_id=profile_id, name_per_lang_code_str=name_per_lang_code_str,
