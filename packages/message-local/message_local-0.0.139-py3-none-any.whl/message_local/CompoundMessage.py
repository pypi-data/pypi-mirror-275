...
class CompoundMessage(GenericCRUD, metaclass=MetaLogger, object=object_message):
    ...
    def set_compound_message_after_text_template(
        ...
        if not recipients:
            ...
            profile_ids = self.criteria_local.get_profile_ids_by_criteria_set_id(criteria_set_ids=criteria_set_ids)
            if not profile_ids:
                raise
            # TODO: develop get_profiles_dicts_by_profile_ids and allow passing select_clause_value
            # TODO: get phone, name, email...
            recipients = []
            for profile_id in profile_ids:
                profile_dict = self.profile_local.get_profile_dict_by_profile_id(profile_id)
                recipients.append(Recipient(profile_id=profile_dict["profile_id"], first_name=profile_dict["title"]))
