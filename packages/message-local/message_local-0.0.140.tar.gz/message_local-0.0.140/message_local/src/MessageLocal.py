...
class MessageLocal(Item, CompoundMessage, metaclass=MetaLogger, object=object_message):
    ...
    def __init__(self, *, recipients: List[Recipient] = None, api_type_id: int = None, campaign_id: int = None,
        ...
        if not self.recipients:
            raise Exception("recipients is required")
    ...
    def get_message_channel(self, recipient: Recipient) -> MessageChannel:
        if recipient.get_profile_id() in self.channel_ids_per_recipient:
            message_channel = self.channel_ids_per_recipient[recipient.get_profile_id()]
        else:
            message_channel = self.get_msg_type(recipient)
    ...
    def after_send_attempt(self, *, sender_profile_id: int = None, outgoing_body: dict or json = None,
                           incoming_message: dict or json = None, http_status_code: int = None,
                           response_body: dict or json or json = None) -> None:
        # TODO: implement sender_profile_id logic
        sender_profile_id = sender_profile_id or self.get_sender_profile_id()
        if self._is_http_api:
    ...
    def __after_indirect_send(self, *, outgoing_body: dict or json, incoming_message: dict or json,
