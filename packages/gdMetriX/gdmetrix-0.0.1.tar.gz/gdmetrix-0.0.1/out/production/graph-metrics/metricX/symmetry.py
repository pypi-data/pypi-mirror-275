...
from metricX import crossings, common, boundary, logConfig
...
def reflective_symmetry(g: nx.Graph, pos: Union[str, dict, None] = None, threshold: int = 10,
    ...
    r"""
    Computes a metric for axial symmetry between 0 and 1 as defined by H. Purchase [1].
    ...
    Note that, with a worst-case runtime of $O(n^7)$ and a best-case runtime of $O(n^5)$,
    the metric is computationally very expensive.

    [1] H. Purchase, "Metrics for Graph Drawing Aesthetics",
    Journal of Visual Languages & Computing 13.5 (2002): 501-516.
    ...
    def _subgraph_area(nodes) -> float:
        ch = ConvexHull([pos[node] for node in nodes])
        return ch.volume
    ...
    for i_a, node_a in enumerate(node_list):
        for i_b in range(i_a, n):
            ...
            if node_a == node_b or pos[node_a] == pos[node_b]:
                continue
...
# TODO use precision for comparisons
...
class _SIFTFeature:
    ...
    def __init__(self, edge: Tuple[numeric, numeric], edge_pos, score):
        ...
        self.angle = ((common.Vector.from_point(edge_pos[1]) - (common.Vector.from_point(edge_pos[0])))
                      .upwardAndForwardDirection().angleToX())
        self.edge = edge
        self.score = score

    def compare_scale(self, other: _SIFTFeature, sigma_scale: float) -> float:
        """
            Calculates the scale facture between two SIFT features in the range of [0,1]
                :param other: The second sift feature to compare to
        :type other: _SIFTFeature
        :param sigma_scale: TODO
        :type sigma_scale: TODO
        :return: A facture between 0 and 1
        :rtype: float
        """
    ...
    def compare_distance(self, other: _SIFTFeature, sigma_distance: float, is_distance_bound: bool):
        if not is_distance_bound:
            return 1
        dist = self.mid.euclidean_distance(other.mid)
...
class _AxesSystem:
    ...
    def __init__(self, pos: dict, sigma_scale: float, distance_bound: bool, sigma_distance: float, num_features: int,
                 angle_merge: float, pixel_merge: float, x_min: int, y_min: int):
        self.pos = pos
        self.y_min = y_min
        self.x_min = x_min
    ...
    def add_rotational_vote_from_two(self, feature_a, feature_b):
        ...
        delta_dist = feature_a.mid.euclidean_distance(feature_b.mid) / 2
        ...
        if abs(radius) < 0.001 or abs(delta_angl) < 0.5 or abs(delta_angl - math.pi) < 0.05:
            ...
            if (test_a.euclidean_distance(feature_b.mid) < 0.0001):
                center = center_a
                factor = -1
            elif (test_b.euclidean_distance(feature_b.mid) < 0.0001):
                center = center_b
                factor = 1
            else:
    ...
    def add_reflective_vote(self, feature):
        ...
        theta_ij = common.Angle((edge_pos_a - edge_pos_b).angleToX() % math.pi)
    ...
    def add_reflective_vote_from_two(self, feature_a, feature_b):
        theta_ij = (feature_a.mid - feature_b.mid).angleToX() % math.pi
        mid = feature_a.mid.mid(feature_b.mid)
    ...
    def _find_maxima(self):
        ...
        # TODO this can be done when adding already, might save us some time/space
...
def visual_symmetry(g: nx.Graph, pos: Union[str, dict, None] = None, resolution: int = 100, sigma: float = 2) -> float:
    """
    ...
    rotational_estimate = 0
    reflective_estimate = 0
    dihedral_estimate = 0
    ...
    for i in range(0, 180, 45):
        ...
        # Rotational symmetry
        rotational_difference_image = rotated_image - graph_image
        ...
        # Reflective symmetry
        reflective_difference_image = flipped_and_rotated - rotated_image
        ...
        _show_image(reflective_difference_image)
    ...
    return 1 - math.pow(min(rotational_estimate, reflective_estimate, dihedral_estimate) / original_sum, 2)
