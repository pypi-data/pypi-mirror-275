...
def get_sgcrf_data(X_train, X_test, y_train, train_ids, test_ids, neighbouring_list, number_of_layers, classification):
    ...
    for layer in range(1,number_of_layers+1):
            ...
            for spatial_id in spatial_ids:
                if layer > 1:
                    ...
                    if spatial_id in temp: temp.remove(spatial_id)
                    layer_matrix_dict[layer][spatial_id] = temp

                    if len(temp) > max_num_neighbours_dict[layer]: max_num_neighbours_dict[layer] = len(temp)
                else:
                    neighbours = layer_matrix_dict[layer][spatial_id]
                    if len(neighbours) > max_num_neighbours_dict[layer]: max_num_neighbours_dict[layer] = len(neighbours)
    ...
    for spatial_id in spatial_ids:
        ...
        for layer in range(1,number_of_layers+1):
            ...
            layer_matrix = layer_matrix_dict[layer]
