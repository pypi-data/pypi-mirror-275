...
<body>
    ...
    <script>
        ...
        document.getElementById('ft-edit').addEventListener('click', async (event) => {{
            ...
            let res = await sendRequest('POST', '/edit', body)
            console.log(res)
...
<body>
    <div class="container">
        ...
        <div class="choose-address">
            <div class="subscribe-datalist">
                <input list="address-datalist-list" id="address-datalist-id" placeholder="EQ: адрес коллекции">
                <datalist id="address-datalist-list">
                </datalist>
            </div>
            <div id="choose-address-id" class="hint">выбери адрес</div>
        ...
        <div class="jetton">
            ...
            <input type="text" id="input-field-name" class="input-field" placeholder="Collection: название коллекции"
                maxlength="128">
            <div id="hint-field-name" class="hint">1-3 слова с заглавной буквы</div>
        ...
        <div class="edit">
            <div class="both">
                <div id="total-title" class="hint">Количество элементов:</div>
                <div id="total-text"></div>
            </div>
            <div class="both">
                <div id="owner-title" class="hint">Подключенный кошелек:</div>
            ...
            <div class="both">
                <div id="master-title" class="hint">Адрес коллекции:</div>
                <div id="master-text">....</div>
            </div>

            <div class="block">
                <input type="number" id="input-mint" class="input-field" placeholder="1: количество NFT"
                    maxlength="256">
                <div id="nft-mint">Отчеканить</div>
            </div>
            <div id="hint-mint" class="hint">создать еще больше NFT</div>
            ...
            <div id="hint-burn" class="hint">удалить NFT из коллекции со своего кошелька</div>

            <div class="block-transfer">
                <div class="block-outer">
                    <div class="block-inner">
                        <input type="number" id="input-amount" class="input-field-second" placeholder="1: номер NFT"
                            maxlength="256" value="1">
                    </div>
                    <input type="text" id="input-address" class="input-field-second" placeholder="EQ: адрес кошелька"
                        maxlength="256">
    ...
    <script>
        ...
        tonConnectUI.onStatusChange(async () => {{
            if (tonConnectUI.connected) {{
                document.getElementById("hint-transfer").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{tonConnectUI.account.address}}`, {{ try_instant_view: true }}) }})
                collections = await sendRequest('POST', '/download', {{ 'dlAddress': dlAddress, 'owner': tonConnectUI.account.address, 'initData': tg.initData }})
                ...
                const createOption = document.createElement('option')
                createOption.value = "+"
                createOption.text = "создать новую колллекцию"
                document.getElementById('address-datalist-list').appendChild(createOption);

                for (const key in collections) {{
                    let collect = collections[key]
                    const option = document.createElement('option')
                    option.value = collect['master']
                    option.text = collect['symbol']
                    document.getElementById('address-datalist-list').appendChild(option)
                }}

                if (collections[activeCollection]['admin'] === collections[activeCollection]['owner'] && collections[activeCollection]['admin'] !== nullAddress) {{
                    document.getElementById('nft-send').style.display = 'none'
                    document.getElementById('nft-edit').style.display = 'flex'
                    document.getElementById('nft-edit').textContent = `Редактировать ${{collections[activeCollection]['symbol']}}`
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            document.getElementById("address-datalist-id").placeholder = "EQ: адрес коллекции"
            document.querySelector('option[value="+"]').textContent = 'создать новую коллекцию';
            document.getElementById("choose-address-id").textContent = 'выбери адрес'
            document.getElementById("input-field-image").placeholder = "https: загрузи лого или вставь ссылку/base64"
            document.getElementById("input-field-name").placeholder = "Collection: название коллекции"
            document.getElementById("hint-field-name").textContent = "1-3 слова с заглавной буквы"
            ...
            document.getElementById("total-title").textContent = 'NFTTFTF'
            document.getElementById("balance-title").textContent = 'Баланс кошелька:'
            document.getElementById("admin-title").textContent = 'Адрес админа:'
            document.getElementById("wallet-title").textContent = 'Адрес кошелька:'
            document.getElementById("owner-title").textContent = 'Подключенный кошелек:'
            ...
            document.getElementById("hint-mint").textContent = 'создать еще больше NFT'
            document.getElementById("hint-admin").textContent = 'передать права на редактирование новому админу'
            document.getElementById("hint-burn").textContent = 'удалить NFT из коллекции со своего кошелька'
            document.getElementById("hint-transfer").textContent = 'отправить несколько NFT другому пользователю'

            document.getElementById("instruction").textContent = 'Инструкция продвижения коллекции'
            document.getElementById("instruction").addEventListener("click", async () => {{ tg.openLink('https://telegra.ph/FereyTONestBot-05-20', {{ try_instant_view: true }}) }})
            document.getElementById("footer").textContent = '@FereyTONestBot'
            document.getElementById("footer").addEventListener("click", async () => {{ tg.openLink('https://t.me/FereyTONestBot', {{ try_instant_view: true }}) }})
        }}) 
        ...
        document.getElementById('address-datalist-id').addEventListener('change', async () => {{
            ...
            document.getElementById("input-field-symbol").value = collections[dlAddress]['symbol']
            document.getElementById("input-field-decimals").value = collections[dlAddress]['decimals']
            document.getElementById("input-field-total").value = collections[dlAddress]['total']
            ...
            document.getElementById("total-text").textContent = `${{collections[activeCollection]['total']}} ${{collections[activeCollection]['symbol']}}`
            document.getElementById("balance-text").textContent = `${{collections[activeCollection]['amount']}} ${{collections[activeCollection]['symbol']}}`
            ...
            document.getElementById("wallet-text").textContent = collections[activeCollection]['wallet_str'].slice(0, 4) + "...." + collections[activeCollection]['wallet_str'].slice(-4)
            document.getElementById("master-text").textContent = collections[activeCollection]['master_str'].slice(0, 4) + "...." + collections[activeCollection]['master_str'].slice(-4)
            ...
            document.getElementById("balance-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{collections[activeCollection]['wallet']}}`, {{ try_instant_view: true }}) }})
            ...
            document.getElementById("wallet-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{collections[activeCollection]['wallet']}}`, {{ try_instant_view: true }}) }})
            ...
            document.getElementById("master-title").addEventListener("click", async () => {{ tg.openLink(`https://tonviewer.com/${{activeCollection}}`, {{ try_instant_view: true }}) }})

            if (collections[dlAddress]['admin'] === collections[dlAddress]['owner'] && collections[dlAddress]['admin'] !== nullAddress) {{
                document.getElementById('nft-send').style.display = 'none'
                document.getElementById('nft-edit').style.display = 'flex'
                document.getElementById('nft-edit').textContent = `Редактировать ${{collections[dlAddress]['symbol']}}`
            ...
            document.getElementById('input-burn').placeholder = `1000: количество ${{collections[activeCollection]['symbol']}}`
            document.getElementById('input-mint').placeholder = `100: количество ${{collections[activeCollection]['symbol']}}`

            document.getElementById('address-datalist-list').innerHTML = ''
            const createOption = document.createElement('option')
            createOption.value = "+"
            createOption.text = "создать новую коллекцию"
            document.getElementById('address-datalist-list').appendChild(createOption)
            ...
            document.getElementById('symbol-dt-id').value = collections[activeCollection]['symbol']
        }})
        ...
        async function editHandle() {{
            ...
            document.getElementById('nft-edit').textContent = `Редактировать ${{symbol}}`
            document.getElementById('input-burn').placeholder = `1000: количество ${{symbol}}`
            document.getElementById('input-mint').placeholder = `100: количество ${{symbol}}`
        }}
        // ----------------------------------------------------
        ...
        document.getElementById('nft-revoke').addEventListener('click', async (event) => {{
            console.log(event.target.id)
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            if (activeCollection === "") {{ tg.showAlert("Выбери адрес мастера, чтобы отозвать права у админа"); return }}
        }})
        ...
        document.getElementById('nft-mint').addEventListener('click', async (event) => {{
            ...
            console.log(collections[activeCollection]['admin'])
            console.log(collections[activeCollection]['owner'])
            ...
            let inputMint = document.getElementById('input-mint').value.trim()
            if (inputMint === "") {{ tg.showAlert("Введи количество NFT для отчеканивания"); return }}

            body = {{
                'owner': tonConnectUI.account.address,
                'master': collections[activeCollection]['master'],
                'wallet': collections[activeCollection]['wallet'],
                'coins': inputMint.toString(),
                'decimals': collections[activeCollection]['decimals'],
                'symbol': collections[activeCollection]['symbol'],
            ...
            collections[activeCollection]['balance'] = (parseInt(collections[activeCollection]['balance'], 10) + parseInt(inputMint, 10)).toString()
            document.getElementById("balance-text").textContent = `${{collections[activeCollection]['balance']}} ${{collections[activeCollection]['symbol']}}`

            collections[activeCollection]['total'] = (parseInt(collections[activeCollection]['total'], 10) + parseInt(inputMint, 10)).toString()
            document.getElementById("total-text").textContent = `${{collections[activeCollection]['total']}} ${{collections[activeCollection]['symbol']}}`
        }})
        ...
        document.getElementById('nft-burn').addEventListener('click', async (event) => {{
            ...
            if (!(tonConnectUI.connected)) {{ tg.showAlert("👩🏽‍💻 Необходимо подключить TON-кошелек"); return }}
            let tmp = document.getElementById('address-datalist-id').value.trim()
            if (activeCollection === "" && tmp === "") {{ tg.showAlert("Создай коллекцию или выбери адрес мастера/кошелька для сжигания NFT"); return }}
            let inputBurn = document.getElementById('input-burn').value.trim()
            ...
            if (Object.keys(collections).length) {{
                collections[activeCollection]['amount'] = parseInt(collections[activeCollection]['amount'], 10) - parseInt(inputBurn, 10)
                document.getElementById("balance-text").textContent = `${{collections[activeCollection]['amount']}} ${{collections[activeCollection]['symbol']}}`
            }}
        }})
