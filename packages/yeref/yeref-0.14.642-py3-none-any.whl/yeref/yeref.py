...
<body>
    ...
    <script>
        ...
        document.querySelectorAll('.input-field').forEach(inputField => inputField.style.background = theme === "DARK" ? getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-secondary-bg-color') : '');
        document.querySelectorAll('.input-field-second').forEach(inputField => inputField.style.background = theme === "DARK" ? getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-secondary-bg-color') : '');
        document.getElementById('symbol-dt-id').style.background = theme === "DARK" ? getComputedStyle(document.documentElement).getPropertyValue('--tg-theme-secondary-bg-color') : '';
...
<head>
    ...
    <style>
        ...
        *::after {{
        ...
        .subscribe-datalist input {{
            width: 100%;
            padding-right: 30px; /* пространство для иконки стрелки */
        }}

        .subscribe-datalist::after {{
            content: '';
            position: absolute;
            right: 10px; /* позиционирование иконки */
            top: 50%;
            transform: translateY(-50%);
            width: 10px; /* размер иконки */
            height: 14px; /* размер иконки */
            pointer-events: none; /* чтобы не мешать кликам по input */
            background-image: url('data:image/svg+xml;utf8,<svg width="10" height="14" viewBox="0 0 10 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M1.6594 10.2122C1.91469 10.5384 2.38607 10.5959 2.71227 10.3406L8.00003 6.20237L13.2878 10.3406C13.614 10.5959 14.0854 10.5384 14.3407 10.2122C14.5959 9.88604 14.5385 9.41465 14.2123 9.15937L8.46226 4.65937C8.19074 4.44688 7.80932 4.44688 7.5378 4.65937L1.7878 9.15937C1.46161 9.41465 1.40412 9.88604 1.6594 10.2122Z" fill="%23909099"/></svg>');
            background-size: contain; /* чтобы иконка полностью помещалась */
            background-repeat: no-repeat; /* чтобы не было повторения иконки */
        }}
    </style>
</head>
    ...
    <div class="container">
        ...
        <div class="choose-address">
            <div class="subscribe-datalist">
                ...
                <datalist id="address-datalist-list">
                    <option value="single">создать единичный NFT</option>
        ...
        <div class="edit">
            ...
            <div class="hint">рекомендуется отозвать прав админа на: отчеканивание новых NFT и передачу прав
    ...
    <script>
        ...
        tonConnectUI.onStatusChange(async () => {{
            if (tonConnectUI.connected) {{
                ...
                document.getElementById('address-datalist-list').innerHTML = ''
                const createOption0 = document.createElement('option')
                createOption0.value = "single"
                createOption0.text = "создать единичный NFT"
                document.getElementById('address-datalist-list').appendChild(createOption0)

                document.getElementById('address-datalist-list').innerHTML = ''
                const createOption = document.createElement('option')
                createOption.value = "album"
                createOption.text = "создать памятный альбом"
                document.getElementById('address-datalist-list').appendChild(createOption)

                const createOption2 = document.createElement('option')
                createOption2.value = "content"
                createOption2.text = "сохранить авторский пост"
                document.getElementById('address-datalist-list').appendChild(createOption2)
        ...
        document.addEventListener("DOMContentLoaded", async () => {{
            ...
            document.querySelector('option[value="single"]').textContent = 'создать единичный NFT'
            document.querySelector('option[value="album"]').textContent = 'создать памятный альбом'
            document.querySelector('option[value="content"]').textContent = 'сохранить авторский пост'
            document.getElementById("choose-address-id").textContent = 'выбери адрес'
            document.getElementById("input-field-image").placeholder = "https: загрузи лого или вставь ссылку/base64"
            ...
            document.getElementById('nft-address').addEventListener('click', async () => {{
                navigator.clipboard.writeText(document.getElementById('nft-address').textContent)
                document.querySelector('.copy-media').classList.add('window--open')
                await sleep(1000).then(() => {{ document.querySelector('.copy-media').classList.remove('window--open') }})
            }})
        }})
        ...
        document.getElementById('address-datalist-id').addEventListener('change', async () => {{
            ...
            document.getElementById('input-mint').placeholder = `UQ: адрес NFT`
        ...
        document.getElementById('gen-rand-nft').addEventListener('click', async () => {{
            ...
            for (let i = 1; i < res['attributes'].length; i++) {{               
                console.log(res['attributes'][i])
                console.log(res['attributes'][i]['trait_type'])
        ...
        document.getElementById('nft-mint').addEventListener('click', async (event) => {{
            ...
            let inputMint = document.getElementById('input-mint').value.trim()
            if (inputMint === "") {{ tg.showAlert("Введи индекс NFT для отчеканивания"); return }}
