...
@dataclass
class File:
    _api: UploadsConnectionProtocol
    id: Optional[str]
    ...
    def delete(self):
        ...
        self._api.delete_file(self.id)

    def reload_status(self):
        """Updates the status of the file."""
        if self.id is None:
            raise ValueError("File not uploaded")
        status_response = self._api.get_file_status(self.id)
    ...
    def download_original(self, filename: str):
        ...
        response = self._api.get_download_presigned_url(self.id)
    ...
    def get_annotated_dict(self):
        ...
        if self.id is None:
            raise ValueError("File not uploaded")
        content_chunks = self._api.download_annotated(self.id)
        content = b"".join(content_chunks)
        return json.loads(content)
    ...
    def _download_text(self, filename: str):
        ...
        if self.id is None:
            raise ValueError("File not uploaded")
        content = self._api.download_text(self.id)
        with open(filename, "wb") as f:
            for chunk in content:
                f.write(chunk)
