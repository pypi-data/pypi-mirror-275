...
def transform_instance_values(instance_values, transformations):
    new_instance_values = []
...
def analyze_shap_values(
    ...
    # shap_values = shap_values_array[index].values
    # starting_value = shap_values_array[index].base_values
    # feature_values = shap_values_array[index].data
    ...
    with doc.create(Section("Feature importance with included context and text", numbering=False)):
        ...
        with doc.create(Subsection("Description of the method", numbering=False)):
            # doc.append(
            #     NoEscape(
            #         r"\begin{tcolorbox}[colback=white, colframe=black, sharp corners, boxrule=0.1mm]"
            #     )
            # )
            # doc.append(NoEscape(r"\setstretch{1.1}"))
    ...
    # doc.append(
    #     NoEscape(
    #         r"\begin{tcolorbox}[colback=white, colframe=black, title=Textual explanations, sharp corners, boxrule=0.3mm, opacityback=0.9, opacityframe=0.99]"
    #     )
    # )
    # doc.append(NoEscape(r"\setstretch{1.1}"))
    doc.append(bold("\nTextual explanation: "))
