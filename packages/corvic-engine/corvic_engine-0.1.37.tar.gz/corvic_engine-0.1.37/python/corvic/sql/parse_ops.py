...
from corvic.well_known_types import VectorSimilartiyMetric
...
def _to_vector_similarity_metric(val: str) -> VectorSimilartiyMetric:
    for arg in get_args(VectorSimilartiyMetric):
        if val == arg:
            return arg
    _logger.warning(
...
class VectorSearchQueryGenerator(Protocol):
    ...
    def __call__(
        ...
        num_results: int,
        similarity_metric: VectorSimilartiyMetric,
    ) -> sqlglot.exp.Query: ...
