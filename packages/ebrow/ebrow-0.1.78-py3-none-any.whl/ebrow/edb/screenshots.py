...
from .utilities import splitASCIIdumpFile, splitBinaryDumpFile, mkExportFolder, addDateDelta, cleanFolder
...
class ScreenShots:
    ...
    def autoExport(self, classFilter):
        ...
        df = self._parent.dataSource.getADpartialCompositeFrame(begDate, self._parent.toDate,
                                                               classFilter)
        firstID = 0
        if df.shape[0] == 0:
            # empty dataframe
            ...
            for idn in ids:
                ...
                if lastingMs >= minLasting:
                    if self._selectCurrentId(idn) > 0:
                        ...
                        if self._settings.readSettingAsBool('aeScreenshot'):
                        ...
                        if self._settings.readSettingAsBool('aeDetails'):
                            ...
                            self._exportPressed(requireManualComments)
                        if self._settings.readSettingAsBool('aePowerPlot'):
                            ...
                            if self._currentObj is not None:
                                ...
                                self._exportPressed(requireManualComments)
                                if self._settings.readSettingAsBool('ae2Dplot'):
                                    print("exporting 2D plot")
                                    self._ui.twShots.setCurrentIndex(self.SSTW_2D)
                                    self._exportPressed(requireManualComments)
                                if self._settings.readSettingAsBool('ae3Dplot'):
    ...
    def _exportPressed(self, checked):
        ...
        if self._ui.twShots.currentIndex() == self.SSTW_CHRONO:
            ...
            if shotName is not None and shotData is not None:
                ...
                defaultComment = "{} ID#{} daily#{} screenshot".format(utcDate, self._parent.currentID, dailyNr)
                if checked:
            ...
            if self._dfDetails is not None:
                ...
                defaultComment = "{} ID#{} daily#{} details".format(utcDate, self._parent.currentID, dailyNr)
                if checked:
            ...
            if prefix is not None:
                ...
                datName, datData, dailyNr, utcDate = self._parent.dataSource.extractDumpData(self._parent.currentID)
                defaultComment = "{}  {} ID#{} daily#{}\n".format(dailyNr, heading, self._parent.currentID, utcDate)
                min, max = self._plot.getMinMax()
