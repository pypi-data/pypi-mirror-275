...
def cicd(
    ...
    comparisons = [
        ComparisonTableConfiguration(name=delta_cicd.delta_name, source_table_id=source.id, target_table_id=target.id)
    ]

    delta = Delta(
        name=delta_cicd.delta_name,
        named_schedule=delta_cicd.cron_schedule.to_datawatch_object(),
    ...
    delta_info = client.get_delta_information(delta_ids=[delta_run.id])

    exit_code = vendor_report.publish(
        delta_cicd.fq_source_table_name, delta_cicd.fq_target_table_name, delta_info[0]
    )
