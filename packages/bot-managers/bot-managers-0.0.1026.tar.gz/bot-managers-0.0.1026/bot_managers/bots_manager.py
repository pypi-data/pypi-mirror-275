...
class ListeningTGBotManager(TGBotManager):
    ...
    async def wait_messages(
            self, tgbot_id: TGBotAbstractModel.tgbot_id, client
    ) -> None:
        pass
    ...
    async def start_handlers(
        ...
        client = await self.get_client(tgbot_id)
        asyncio.create_task(
            self.wait_messages(tgbot, tgbot_id, client),
            name=f'wait_messages_{tgbot_id}'
        )
    ...
    async def process_update_listener_tgbot(
            self, tgbot_id: TGBotAbstractModel.tgbot_id, at_start: bool = False
    ) -> None:
        if not isinstance(tgbot_id, ShuttingDown):
            ...
            async for tgbot_id in self.client_manager.clients:
                await self.stop_handlers(tgbot_id)
            await self.client_manager.process_update_tgbot(tgbot_id)
