...
class TGBotManager:
    ...
    def __init__(
        ...
        self.number = number
        self.manager_name = ManagerType.RESPONSIBLE if self.responsible else ManagerType.LISTENING
    ...
    async def close_all_tasks_twice(self, first_time: bool = True) -> None:
        tasks = [t for t in asyncio.all_tasks(self.loop) if
        ...
        for task in tasks:
            if not task.done():
                self.logger.close_all_tasks(
                    'info',
                    f'Task {task} did not complete correctly. {first_time=}'
                )
        if first_time:
            await self.close_all_tasks_twice(first_time=False)

    async def aio_stop(self):
        await self.close_all_tasks_twice()
...
class ResponsibleTGBotManager(TGBotManager):
    ...
    async def start_tasks(self) -> None:
        ...
        for queue_suffix, callback, kwargs in self.tasks_config:
            queue_name = (f'{self.rabbitmq_settings.prefix}{queue_suffix}'
                          f'_{self.number}')
            tasks.append(
                self.rabbitmq.consume_queue(
