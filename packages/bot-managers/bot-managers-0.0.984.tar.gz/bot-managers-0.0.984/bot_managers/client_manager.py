...
class TelethonClientManager:
    ...
    async def _safe_create_telegram_client(self, tgbot: TGBotAbstractModel):
        ...
        for attempt in range(self.try_limit):
            ...
            try:
                ...
                self.logger.try_create_telegram_client('error', msg+str(exc))
    ...
    async def _run_client(self, client, tgbot):
        for attempt in range(self.try_limit):
            ...
            try:
                ...
                self.logger.log_run_client('info')
                print('success !!!!!!!!!!!!!!!!!!!!')
                return True
        return False

    async def _end_client_session(self, tgbot):
        if tgbot.tgbot_id in self.clients:
            self.clients[tgbot.tgbot_id]['status'] = 'reconnecting'
            client = await self.get_client(tgbot.tgbot_id, get_for_update=True)
            await self.disconnect_client(client)
            client.session.close()
    ...
    async def process_update_tgbot(self, tgbot: TGBotAbstractModel, at_start=False) -> None:
        if isinstance(tgbot, ShuttingDown):
            await self.aio_stop()
            return
        for attempt in range(self.try_limit):
            if not at_start:
                await self._end_client_session(tgbot)
            else:
    ...
    async def reconnect_client(self, tgbot):
        client_info = await self._get_client_info(tgbot.tgbot_id)
        if client_info['status'] == 'reconnecting':
            return
        self.logger.get_update_tgbot('info', tgbot)
        if tgbot.is_activated:
            await self.send_msgs_for_update_client(tgbot)
        else:
            await self._end_client_session(tgbot)
    ...
    @abc.abstractmethod
    async def _get_tgbot(self, tgbot_id):
        pass

    async def get_client(self, tgbot_id, get_for_update=False):
        tgbot = await self._get_tgbot(tgbot_id)
        for attempt in range(self.try_limit):
            client_info = await self._get_client_info(tgbot_id)
            try:
                ...
                if not client.is_connected() or not await self._check_client(
                    ...
                    await self.reconnect_client(tgbot)
                    continue
