...
def get_lang(message: str, i: int, end: int) -> tuple[str, str, int]:
    """
    Extracts the language of a code block from a message.
    :param message: the message to extract the language from.
    :param i: the start of the code block.
    :param end: the end of the code block.
    :return: a tuple consisting of (language, message, end).
    """
    lang = ''
    ...
    if not start_code == -1:
        unvalidated_lang = message[i:start_code].strip()
        # Validate language against regex; reset if not matching
        if re.compile(r'[a-zA-Z0-9_-]{1,32}'
                      ).fullmatch(unvalidated_lang) is not None:
            lang = unvalidated_lang
            code_block = message[start_code:end].strip()
            message = message[:i] + code_block + message[end:]
        ...
        if m:
            ...
            if end != -1:
                ...
                if ent == MessageEntityPre:
                    ...
                    lang, message, end = get_lang(message, i, end)
                    print(f'{lang=}, {i=}, {message=}')
