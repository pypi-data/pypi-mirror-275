...
original_decrypt_message_data = MTProtoState.decrypt_message_data


def patched_decrypt_message_data(instance, body):
    """
    Inverse of `encrypt_message_data` for incoming server messages.
    """
    try:
        original_decrypt_message_data(instance, body)
    except SecurityError as exc:
        # A step while decoding had the incorrect dat a. This message.
        if exc.args[0] == ('Server replied with a wrong session ID '
                           '(see FAQ for details)'):
            logger = BaseLogger()
            logger.patched_decrypt_message_data('error', exc.args[0])
            raise CustomSecurityError(exc.args[0])


MTProtoState.decrypt_message_data = patched_decrypt_message_data
