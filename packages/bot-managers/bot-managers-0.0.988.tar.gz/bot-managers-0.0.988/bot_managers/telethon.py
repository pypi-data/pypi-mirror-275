...
class PathedMTProtoState(MTProtoState):
    def decrypt_message_data(self, body):
        """
        Inverse of `encrypt_message_data` for incoming server messages.
        """
        try:
            super().decrypt_message_data(body)
        except SecurityError as exc:
            # A step while decoding had the incorrect dat a. This message.
            if exc.args[0] == ('Server replied with a wrong session ID '
                               '(see FAQ for details)'):
                logger = BaseLogger()
                logger.decrypt_message_data('error', exc.args[0])
                raise CustomSecurityError(exc.args[0]) from exc


class PathedMTProtoSender(MTProtoSender):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._state = PathedMTProtoState(self.auth_key, loggers=self._loggers)
