<script lang="ts">
	...
	const is_child_path = (path: string[], path_2: string[]): boolean => {
		if (path_2.length === path.length - 1) {
			return path_2.join("/") === path.slice(0, path_2.length).join("/");
		}
		return false;
	};
	...
	function set_content(path: string[], all_files: FileNode[]) {
		let new_content = [];
		if (path.length === 0) {
			new_content = all_files.filter((x) => x.path.length === 1);
		} else {
			new_content = all_files.filter((x) => is_child_path(x.path, path));
		}
		if (valid_for_selection) {
			new_content = [{ name: ".", type: "file" }, ...new_content];
		}
		return new_content;
	}

	function set_open_folders(content: FileNode[]) {
		const new_opened_folders = content
			.map((x, i) =>
				x.type === "folder" &&
				(is_selected_entirely ||
					selected_files.some((y) => y[0] === x.name))
					? i
					: null,
			)
			.filter((x): x is number => x !== null);
		return new_opened_folders;
	}
	...
	$: content = set_content(path, all_files);
	$: opened_folders = set_open_folders(content);
