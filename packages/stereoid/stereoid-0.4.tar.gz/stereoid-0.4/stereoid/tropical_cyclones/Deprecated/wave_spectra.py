...
def Kudry_spec( kx, ky, u_10, fetch, phi_w, dks ):
    ...
    B_neq = k ** 4 * S
    # FIXME: I changed this function, check what the difference is (also, is 10*k_p the correct limit )
    f = phi_func( (k / (10*k_p)) ** 2 )  # np.where( k > 0, (1 + np.tanh( 2 * (np.log( k ) - np.log( 10*k_p )) )) / 2, 0 )
...
# based on A10 in Kudry et al. (2014)
def phi_func( x ):
    f = x ** 4 / (1 + x ** 4)
    return f
...
######################### JONSWAP ########################
# Based on Jocelyn Frechot (2006). Realistic simulation of ocean surface using wave spectra
def jonswap( k, phi, omega_p, phi_w, alpha = 0.0081, gamma = 3.3 ):
    # Input:
