...
class DeepchecksLLMClient:
    ...
    def get_interactions_complete_status(
        ...
        Parameters
        ----------
        events_to_check (Optional[List[InteractionCompleteEvents]]): A list of events whose completion status needs to be checked.
        By default, only annotation_completed is checked
        user_interaction_ids (List[str]): A list of user interaction IDs to check for completion status.

        Returns
        -------
        Dict[InteractionCompleteEvents, {"all_completed": bool, "number_of_completed": int]:
        A dictionary where keys are events from `events_to_check`
        and values are objects that indicating whether interactions for each event are completed and number of completed interactions
        {"annotation_completed": {"all_completed": True, "number_of_completed": 10}}
        """
    ...
    def get_data_if_calculations_completed(
        ...
        max_retries (int, optional): Maximum number of retries if calculations are not completed. Defaults to 60.
        retry_interval_in_seconds (float, optional): Interval between retries in seconds. Defaults to 20.

        Returns
        -------
        Optional[pd.DataFrame]: A DataFrame containing the retrieved data if calculations are completed,
        otherwise None.
        """
        if not events_to_check:
            events_to_check = [InteractionCompleteEvents.ANNOTATION_COMPLETED]
