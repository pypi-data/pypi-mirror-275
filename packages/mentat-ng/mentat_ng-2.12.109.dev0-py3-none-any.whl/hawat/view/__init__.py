...
class ItemActionView(RenderableView):  # pylint: disable=locally-disabled,abstract-method
    ...
    def inform_about_filter_change(self, _filter, author, diff, managers, maintainers=None):
        ...
        """
        def _send_email(to, template_path, locale=None):
        ...
        def _inform_users(users, template_path):
            for user in users:
                if user in informed_users:
                    continue

                _send_email(to=[user.email], template_path=template_path, locale=user.locale)
        ...
        try:
            ...
            if maintainer_emails_override:
                ...
                _send_email(maintainer_emails_override, template_path='filters/email_maintainers.txt')
            else:
                if not maintainers:
                ...
                _inform_users(users=maintainers, template_path='filters/email_maintainers.txt')
        ...
        try:
            _inform_users(users=managers, template_path='filters/email_managers.txt')
        except ConnectionRefusedError:
            # Mail service is probably not configured.
