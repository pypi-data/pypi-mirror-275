...
def read_s3_file(
    ...
    try:
        ...
        elif url.endswith(".orc") or url.endswith(".orc.sz"):
            logger.info(f"Reading {num_rows_to_sample} rows from S3 file: {url}")
...
def get_schemas_from_files(
    client, bucket: str, filepath: str, row_sample_count: Optional[int]
) -> list[dict]:
...
def detect_s3_data_assets_history(
    ...
    schema_historical = get_schemas_from_files(
        client, strip_s3_bucket_prefix(bucket), first_pattern, row_sample_count
    )
    schema_today = get_schemas_from_files(
        client, strip_s3_bucket_prefix(bucket), second_pattern, row_sample_count
    )
...
def register_s3_data_assets(
    ...
    pattern_to_schema_and_profiles = detect_s3_data_assets(
        bucket, skip_profiling, lookback_days, row_sample_count, include
    )
