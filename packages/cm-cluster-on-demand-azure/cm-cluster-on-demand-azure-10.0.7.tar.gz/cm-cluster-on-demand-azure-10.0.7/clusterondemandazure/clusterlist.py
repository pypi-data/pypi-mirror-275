...
def _get_node_info(azure_api, r_group, name: str, global_vmsizes) -> NodeInfo | None:
    try:
...
def _get_vm_ips(azure_api, r_group, vm, ip_name):
...
def get_cluster_data(azure_api, r_group, global_vmsizes):
    ...
    head_node_a_name = cluster_name
    head_node_b_name = cluster_name + _HA_NODE_B_SUFFIX

    head_node_a = _get_node_info(azure_api, r_group, head_node_a_name, global_vmsizes)
    head_node_b = _get_node_info(azure_api, r_group, head_node_b_name, global_vmsizes)

    if head_node_a is not None:
        public_ip_a, private_ip_a = _get_vm_ips(
            azure_api, r_group, head_node_a.vm, "head-node-public-ip"
        )
    else:
    ...
    if head_node_b is not None:
        public_ip_b, private_ip_b = _get_vm_ips(
            azure_api, r_group, head_node_b.vm, f"{head_node_b_name}-ip"
        )
    else:
