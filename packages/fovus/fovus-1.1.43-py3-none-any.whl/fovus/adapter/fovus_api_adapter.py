    ...
    def _confirm_enable_preemptible_support(self, request):
        ...
        if (
            ...
            print(
                ...
                + f"{max_walltime_allowed} hour. Preemptible resources are subject to reclaim by "
            ...
            if input() == "y":
                ...
                if request[PAYLOAD_CONSTRAINTS][PAYLOAD_TASK_CONSTRAINTS][WALLTIME_HOURS] > max_walltime_allowed:
                    print(
                        "Warning: Preemptible resources will restrict the maximum task Walltime allowed to "
                        + f"{max_walltime_allowed} hour. Overriding Walltime from "
                        + f"{request[PAYLOAD_CONSTRAINTS][PAYLOAD_TASK_CONSTRAINTS][WALLTIME_HOURS]} "
                        + f"to {max_walltime_allowed} hour."
                    )
                    request[PAYLOAD_CONSTRAINTS][PAYLOAD_TASK_CONSTRAINTS][WALLTIME_HOURS] = max_walltime_allowed
            else:
                request[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS][ALLOW_PREEMPTIBLE] = False
