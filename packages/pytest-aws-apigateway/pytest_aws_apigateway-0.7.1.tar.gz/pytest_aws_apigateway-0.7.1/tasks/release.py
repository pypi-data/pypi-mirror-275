...
REMOTE = "felixscherz/pytest-aws-apigateway.git"
...
def get_upstream(repo: Repo) -> Remote:
    for remote in repo.remotes:
        if any(url.endswith(REMOTE) for url in remote.urls):
            return remote
    msg = f"could not find {REMOTE} remote"
    raise RuntimeError(msg)
...
def release_changelog(repo: Repo, version: Version) -> Commit:
    ...
    check_call(
        ["towncrier", "build", "--yes", "--version", version.public],
