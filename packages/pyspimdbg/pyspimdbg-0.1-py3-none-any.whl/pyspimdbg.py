...
class Spim(object):
    ...
    """def interactive(self):
        ...
        while True:
            ...
            if command != '':
                # self.last = command
                last_non_logging_command = command
            else:
                command = last_non_logging_command

            # Exit the loop if the user types 'exit' or 'q'
            if command.lower() == 'exit' or command.lower() == 'q':
                break

            # Send the command to spim
            # print("Sending command to spim... ", command)
            self._sendline(command)

            # Wait for spim to process the command
            index = self._expect(['.*\(spim\) ',
                                 pexpect.EOF,
            ...
            if index == 0:
                ...
                # Remove the last line if command is not newline
                if command != '\n':
                    lines = lines[1:-1]


                # Print the registers if the last command was not 'print_all_regs'
                if last_non_logging_command != 'print_all_regs':
                    self.print_all_regs()
                # Print the result
                print('\n'.join(lines))
            elif index == 1:
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 test.py <filename.s>")
        sys.exit(1)
