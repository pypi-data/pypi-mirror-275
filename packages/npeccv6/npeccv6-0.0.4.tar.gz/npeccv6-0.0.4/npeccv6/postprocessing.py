...
def calculate_root_length(
    ...
    """
    logging.info("Calculating root lengths for a subgraph.")
    ...
    logging.debug(f"Starting node for root length calculation: {highest_node}")
...
def calculate_root_lengths(graph: pd.DataFrame) -> Dict[int, Dict[str, float]]:
    ...
    """
    logging.info("Calculating root lengths for all skeletons.")

    unique_values = graph["skeleton-id"].unique()
    results = {}

    for skeleton_id in unique_values:
        logging.info(f"Processing skeleton ID: {skeleton_id}")
        ...
        main_root_length, lateral_root_length, main_root_src, main_root_dest = (
...
def mark_landmarks(
    ...
    """
    logging.info("Marking landmarks on the image.")
