...
class App:
    ...
    def my_layout(self):
        nodes = [
            flow.Node(id="1", type="input", data=flow.NodeData(label="Right Click On Pane!"), position=flow.XYPosition(0, 0)),
        ]
        items = [
    ...
    async def add_node(self, data):
        ...
        new_node = flow.Node(type=node_type, id=self.graph.create_unique_node_id("node"), data=flow.NodeData(label=node_type), 
            position=flow.XYPosition(mouse_x, mouse_y))
        # screen_to_flow: convert screen position (mouse xy) to flow position
