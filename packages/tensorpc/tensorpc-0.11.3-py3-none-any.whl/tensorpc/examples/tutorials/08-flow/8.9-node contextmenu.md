...
class App:
    ...
    def my_layout(self):
        nodes = [
            flow.Node(id="1", type="input", data=flow.NodeData(label="Right Click On Me!"), position=flow.XYPosition(0, 0)),
            flow.Node(id="2", type="default", data=flow.NodeData(label="Right Click On Me!"), position=flow.XYPosition(0, 100)),
            flow.Node(id="3", type="output", data=flow.NodeData(label="Right Click On Me!"), position=flow.XYPosition(0, 200)),
    ...
    async def change_node(self, data):
        ...
        if item_id == "delete":
            ...
            # here we use position from existing node, so we don't need to convert screen position to flow position
