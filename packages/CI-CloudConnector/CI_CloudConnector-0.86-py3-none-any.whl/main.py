...
class MainFileChangeHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        if event.src_path.endswith(self.main_file):
            print("Main file has been modified. Recreating executable...")
            recreate_executable()


def recreate_executable():
    # Path to the main script
    main_script = "main.py"  # Change this to the name of your main script

    # Path to PyInstaller executable
    pyinstaller_path = "pyinstaller"  # Change this if PyInstaller is not in PATH

    # Command to recreate the executable
    command = [pyinstaller_path, "--onefile", main_script]
    print(pyinstaller_path)
    print(main_script)
    # Run PyInstaller to recreate the executable
