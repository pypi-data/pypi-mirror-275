...
def get_cloud_tags(token=""):
    ...
    try:
        ...
        ci_print(f"Last Get tags from server was {getTagsTimePass} seconds ago, getTagsRate = {GetTagsFromServerMinRateSeconds}", "INFO")
        ...
        if getTagsTimePass == 0 or getTagsTimePass > GetTagsFromServerMinRateSeconds:
            ...
            if ret and ret["isOK"] == True:
                ...
                ci_print(f"Received {len(arranged_tags)} Tags from Cloud", "INFO")
            else:
...
def printTags(tags):
    try:
        ci_print(f"Print Tags: List Contains {len(tags)} Tags", "INFO")
        ci_print(str(tags))
