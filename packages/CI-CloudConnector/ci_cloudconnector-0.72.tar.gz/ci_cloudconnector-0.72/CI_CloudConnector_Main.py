import CI_CloudConnector_Logic as logic, datetime
...
def update_and_run(script_path, package_name, package_version):
    try:
        ...
        # Copy logic.py and main.py from the package to your project
        installed_package_path = os.path.join(package_location, package_name.replace("-", "_"))
        destination_path = os.path.dirname(script_path)

        logic_source = os.path.join(installed_package_path, "CI_CloudConnector_Logic.py")
        main_source = os.path.join(installed_package_path, "CI_CloudConnector_Main.py")
        ...
        # Run the Python script with the specified command-line argument
        subprocess.check_call([sys.executable, script_path] + sys.argv[1:])
...
def upgradeLC(ver="", currentVer=""):
    try:
        ...
        update_and_run("main.py", "CI_CloudConnector", "0.71");
