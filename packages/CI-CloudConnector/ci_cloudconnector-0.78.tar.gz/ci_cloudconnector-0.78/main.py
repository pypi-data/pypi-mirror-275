...
def update_and_run(script_path, package_name, package_version):
    try:
        ...
        # Install required dependencies listed in setup.py
        with open(os.path.join(installed_package_path, 'setup.py')) as setup_file:
            setup_content = setup_file.read()
        required_packages = []
        for line in setup_content.splitlines():
            if line.strip().startswith('install_requires='):
                # Extract the dependencies listed
                start_index = line.index('[') + 1
                end_index = line.index(']')
                required_packages = line[start_index:end_index].replace('"', '').replace("'", '').split(',')
        ...
        # Open a new command prompt and run the script after the current process exits
        #subprocess.Popen(['start', 'cmd', '/k', 'python', 'main.py', 'Start'], shell=True)
        subprocess.Popen(['cmd.exe', '/k', 'python', 'main.py', 'Start'])
