...
class FoundationModelAPIClient:
    def __init__(self, *, model, api_key=None, system_prompt, system_prompt_type, user_prompt, user_prompt_type, user_data=None, user_data_type, output_file=None, temperature=None, max_tokens=None, log_options=None):
        #
        ...
        self.user_data_absolute_path = os.path.abspath(user_data) if user_data else None
        self.output_file_absolute_path = os.path.abspath(output_file) if output_file else None

        random_uuid = uuid.uuid4()

        logging_data = self.process_logging_options()
        ...
        if logging_data["system_prompt_content"] is not None:
            output_data["arguments"]["system_prompt_content"] = logging_data["system_prompt_content"]

        if logging_data["user_prompt_content"] is not None:
            output_data["arguments"]["user_prompt_content"] = logging_data["user_prompt_content"]

        if logging_data["user_data_file_content"] is not None:
            output_data["arguments"]["user_data_file_content"] = logging_data["user_data_file_content"]
    ...
    def process_logging_options(self):
        system_prompt_content = None
        user_prompt_content = None
        user_data_file_content = None

        if self.log_options is None:
            return {
                "system_prompt_content": system_prompt_content,
                "user_prompt_content": user_prompt_content,
                "user_data_file_content": user_data_file_content
            }

        log_options = self.log_options.split(',')
        if 'all' in log_options:
            log_options = ['systemprompt', 'userprompt', 'userdata']

        if 'systemprompt' in log_options:
            system_prompt_content = self.system_prompt_data

        if 'userprompt' in log_options:
            user_prompt_content = self.user_prompt_final_data

        if 'userdata' in log_options and self.user_data_data:
            user_data_file_content = self.user_data_data
...
def main():
    ...
    parser.add_argument('--log', type=str, default=None, help='Various logging options all,systemprompt,userprompt,userdata')
