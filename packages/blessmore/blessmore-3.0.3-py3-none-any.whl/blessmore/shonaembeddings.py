...
def load_fasttext_model(dimension, repo_id="Blessmore/Fasttext_embeddings"):
    ...
    outer_model_dir = f"Fast_text_{dimension}_dim"
    inner_model_dir = os.path.join(outer_model_dir, outer_model_dir)
    model_files = [
    ...
    model_path = os.path.join(inner_model_dir, f"shona_fasttext_{dimension}d.model")
    try:
