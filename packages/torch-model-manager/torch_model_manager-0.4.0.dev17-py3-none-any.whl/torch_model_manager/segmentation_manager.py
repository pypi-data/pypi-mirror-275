...
class SegmentationManager:
    def __init__(self, sam_backbone="sam_vit_h", g_dino_backbone="swin_t", device=DEVICE):
        ...
        if g_dino_backbone == "swin_t":
            ...
            local_config_path = os.path.abspath(os.path.expanduser(g_dino_model_config_path))

            if not os.path.exists(local_config_path):
                os.makedirs("/".join(local_config_path).split("/")[:-1])
                os.system(f"cp GroundingDINO/groundingdino/config/{config_name} {local_config_path}")
