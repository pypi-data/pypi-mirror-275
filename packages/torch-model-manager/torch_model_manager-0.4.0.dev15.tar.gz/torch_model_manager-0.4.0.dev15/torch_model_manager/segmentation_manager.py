...
class SegmentationManager:
    def __init__(self, sam_backbone="sam_vit_h", g_dino_backbone="swin_t", device=DEVICE):
        ...
        if g_dino_backbone == "swin_t":
            ...
            if not os.path.exists(g_dino_model_config_path):
                os.makedirs("/".join(os.path.abspath(os.path.expanduser(g_dino_model_config_path)).split("/")[:-1]))
                os.system(f"cp GroundingDINO/groundingdino/config/{config_name} {os.path.abspath(os.path.expanduser(g_dino_model_config_path))}")
