...
def get_pip_pkgs(pip: Union[str, List[str]]) -> List[str]:
    ...
    # Adding `covalent-cloud` to the list of pip packages to be installed here.
    CC_PKG = "covalent-cloud"

    cc_exists = False
    for pkg in pip_pkgs:
        # Check if `covalent-cloud` is already in the list of pip packages.
        if CC_PKG in pkg:
            cc_exists = True
            break

    if not cc_exists:
        pip_pkgs.append(CC_PKG)
