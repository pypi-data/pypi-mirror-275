...
class WA:
    ...
    def sql_connection(self, uid, pwd, server, database='Vista_Replication_PRD', driver='{SQL Server}'):
        st = f'DRIVER={driver};SERVER={server};DATABASE={database};UID={uid};PWD={pwd};TrustServerCertificate=yes'
        try:
            self.connSQl = odbc.connect(st)
        except Exception as erro:
            import sys
            print(f'Erro de conexão com BD: {erro}')
            sys.exit()

    def sql_connection_old(self, uid, pwd, server, database='Vista_Replication_PRD', driver='ODBC Driver 17 for SQL Server'):
