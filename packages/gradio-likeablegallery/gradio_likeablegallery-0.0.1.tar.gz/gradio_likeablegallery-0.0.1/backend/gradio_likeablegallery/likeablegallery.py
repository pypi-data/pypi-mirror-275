...
@document()
class LikeableGallery(Component):
    """
    ...
    def __init__(
        self,
        value: (
            List[GalleryImageType | CaptionedGalleryImageType | GalleryImage]
            | Callable
            | None
        ) = None,
        *,
    ...
    def postprocess(
        self,
        value: (
            List[GalleryImageType | CaptionedGalleryImageType | GalleryImage | dict]
            | None
        ),
        # **kwargs
    ) -> GalleryData:
    ...
    def preprocess(self, payload: GalleryData | None) -> List[GalleryImage] | None:
        if payload is None or not payload.root:
            return None
        for gallery_element in payload.root:
            image = self.convert_to_type(
                gallery_element.image.path, self.type
            )  # type: ignore
