			...
			{#if likeable}
				<div class="like-button">
					<span style="margin-right: 1px;">
						<IconButton
							size="large"
							highlight={selected_image.liked}
							Icon={Like}
							on:click={(event) => {
								event.stopPropagation();
								if (selected_image.liked) {
									selected_image.liked = undefined;
									dispatch("like", {
										index: selected_index,
										value: selected_image.image,
										liked: selected_image.liked,
									});

									return;
								}
								selected_image.liked = true;
								dispatch("like", {
									index: selected_index,
									value: selected_image.image,
									liked: selected_image.liked,
