...
class HistoryRowData:
    ...
    tx_id: int  # ID транзакции.
    change_type: int  # Тип изменения. 1 - INSERT, 2 - UPDATE, 3 - DELETE
    table_name: str  # Имя таблицы.
    key: Union[int, List[Union[int, str]]]  # Ключ записи таблицы.
    seq_number: int  # Порядковый номер изменения записи в рамках транзакции.
    ...
    def __generate_record_class(self, data: dict) -> object:
        ...
        for field_name, value in data.items():
            field_value = HistoryRecordField(field_name=field_name, value=value, table_name=self.table_name)
    ...
    def __prepare_history_rows(self, history_rows):
        ...
        for row in history_rows:
            data_kwargs = {
                ...
                if key in list(HistoryRowData.__dataclass_fields__.keys())
            }
