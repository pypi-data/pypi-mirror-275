...
const Cell = (props: ICellPageProps): JSX.Element => {
  ...
  const setAndFilterData = (data: any): void => {
  ...
  const fetchDataLazyLoading = () => {
    fetchWithCredentials(
      `${BACKEND_API_URL}/dashboard/${props.notebookId}/cell/${content.cellId}?limit=${limit}&offset=${offsetRef.current}&${generateQueryArgsString(dashboardQueryArgsRedux, props.notebookId)}`
    )
      ...
      .then(data => {
        ...
        const mergedData = [...originalData.current, ...data];
        originalData.current = mergedData;
        setAndFilterData(mergedData);
