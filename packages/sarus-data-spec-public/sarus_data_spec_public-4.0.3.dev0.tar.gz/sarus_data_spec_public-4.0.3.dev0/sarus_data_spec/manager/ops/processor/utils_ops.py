...
from sarus_data_spec.manager.async_utils import (
    is_async_iterator_empty,
    async_iter,
)
import sarus_data_spec.typing as st


async def ensure_batch_is_not_empty(
    dataset: st.DataSpec,
    ...
    is_empty, original_async_iterator = await is_async_iterator_empty(
        async_iterator
    )
    if not is_empty:
    ...
    If you believe that the computation on real data is not empty, you may provide the schema by adding a dummy example using the parameter 'sarus_default_output'."""
        raise ValueError(message_empty_ait)
