                ...
                with st.echo():
                    ...
                    with dt_container:
                        dt_response = st_datatable(
                            big_data, column_order=column_order, props=props, table_id="example_1", key="example_1", dt_config="default", date_format="%m/%d/%Y", disabled=False, export_file_name="Example 1", on_click=my_on_click_function, args=["example_1"])
                        st.json(dt_response)
        with st.container(border=True):
