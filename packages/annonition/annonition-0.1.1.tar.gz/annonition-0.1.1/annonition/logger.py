...
class CustomLogger:
    ...
    class ProgressSpinner:
        ...
        def __init__(self, message: str, timer: bool=True):
    ...
    def spinner(self, message: str, timer: bool=True):
        """
        Returns a context manager for displaying a progress spinner with the given message.

        Usage:
        ```python
        # Timer is optional, and is True by default.
        with logger.spinner("Doing things...", timer=True) as progress:
            time.sleep(1)
            progress.change("Still doing things...")
            time.sleep(1)
            progress.log("Finished 1 thing.")
        ...
        ```
        '''
        Finished 1 thing. (2.00s)
        💥  Couldnt do everything. (3.00s)
        '''
        ```
        ...
        """
        return self.ProgressSpinner(message, timer)
    ...
    def timer(self, task_name: str, type: Optional[TimerType] ="info"):
        ...
        Usage:
        ```python
        with logger.timer("Doing something..."):
            time.sleep(5)
        ```
        Logs:
        ...
        try:
            ...
            if type == "print":
                print(f"{task_name}: {elapsed_time:.4f}s")
            elif type =="debug":
                self.adapter.debug(f"{task_name}: {elapsed_time:.4f}s")
            elif type == "info":
                self.adapter.info(f"{task_name}: {elapsed_time:.4f}s")
            elif type == "warning":
                self.adapter.warning(f"{task_name}: {elapsed_time:.4f}s")
            elif type == "error":
                self.adapter.error(f"{task_name}: {elapsed_time:.4f}s")
            elif type == "critical":
                self.adapter.critical(f"{task_name}: {elapsed_time:.4f}s")
