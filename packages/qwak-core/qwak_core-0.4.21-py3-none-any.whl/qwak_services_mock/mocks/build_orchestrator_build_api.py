...
from _qwak_proto.qwak.build.v1.build_api_pb2 import (
    ...
    UpdateBuildStatusResponse, SaveFrameworkModelsRequest, SaveFrameworkModelsResponse,
)
...
from qwak.clients.build_orchestrator.client import HuggingModelDataClass, FrameworkModelDataClass
...
class BuildOrchestratorBuildApiMock(BuildAPIServicer):
    ...
    def ListBuilds(self, request: ListBuildsRequest, context) -> ListBuildsResponse:
        valid_builds = [self._builds[buildId] for buildId in self._builds if
                        self._builds[buildId].model_uuid == request.model_uuid]
        builds_to_return = []
