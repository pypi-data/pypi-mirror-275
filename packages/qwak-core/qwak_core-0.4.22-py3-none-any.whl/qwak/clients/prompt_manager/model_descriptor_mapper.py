    ...
    def from_prompt_openai_provider(
        ...
        if descriptor.capabilities.WhichOneof("optional_chat"):
            return ModelDescriptorMapper.from_openai_chat_capability(
                model_id=model_id,
                openai_chat_params=descriptor.capabilities.chat_params,
            )
        else:
            raise QwakException(
    ...
    def to_openai_chat(model_descriptor: OpenAIChat) -> ProtoOpenAIModelDescriptor:
        ...
        model_capabilities = ProtoOpenAICapabilities(
            chat_params=ProtoOpenAiChatModelParams(
                frequency_penalty=d.frequency_penalty if d.frequency_penalty else None,
                logit_bias=logit_bias_struct if d.logit_bias else None,
                logprobs=d.logprobs if d.logprobs else None,
                max_tokens=d.max_tokens if d.max_tokens else None,
                n=d.n if d.n else None,
                presence_penalty=d.presence_penalty if d.presence_penalty else None,
                response_format=d.response_format
                if d.response_format
                else None,  # noqa
                seed=d.seed if d.seed else None,
                stop=stop_list_value if d.stop else None,
                temperature=d.temperature if d.temperature else None,
                tool_choice=tool_choice_proto if d.tool_choice else None,
                tools_spec=tools_proto if d.tools else None,
                top_logprobs=d.top_logprobs if d.top_logprobs else None,
                top_p=d.top_p if d.top_p else None,
