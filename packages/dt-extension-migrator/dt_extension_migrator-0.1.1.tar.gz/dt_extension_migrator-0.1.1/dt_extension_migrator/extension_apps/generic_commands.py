...
def build_ef2_config_from_ef1(
    ...
    # if merge_commands:
    #     grouped_commands = ef1_configurations.groupby("hostname")
    #     for key, group in grouped_commands:
    #         base_config = None
    #         for index, row in group.iterrows():
    #             if not base_config:
    #                 base_config = row['properties']
    #             else:
    ...
    for index, row in ef1_configurations.iterrows():
        ...
        command = {
            ...
            "frequency": int(properties.get("frequency")) if properties.get("frequency") else 15,
            "location": properties.get("location") if properties.get("location") else "ActiveGate",
        ...
        if properties.get("output_evaluation_behavior") == "TEXT_PATTERN_MATCH":
            ...
            command["output_validation_pattern"] = properties.get("output_validation_pattern")
            ...
            command["output_validation_numeric_operator"] = CompareOperator(properties.get("output_validation_numeric_operator")).name
            command["output_validation_numeric_value"] = properties.get("output_validation_numeric_value")
        ...
        if merge_commands:
            if not properties.get("hostname") in hostname_merged_commands:
                hostname_merged_commands[properties['hostname']] = endpoint_configuration # first hostname config we see is the base
            else:
                hostname_merged_commands[properties.get("hostname")]['commands'].append(command)
        else:
    ...
    if merge_commands:
        for host in hostname_merged_commands:
            print(host, hostname_merged_commands[host])
            base_config["pythonRemote"]["endpoints"].append(hostname_merged_commands[host])   
    return base_config
...
def pull(
    ...
    index: Annotated[Optional[List[str]], typer.Option(help="Specify what property to group sheets by. Can be specified multipl times.")] = ["group"],
):
    dt = Dynatrace(dt_url, dt_token)
    configs = dt.extensions.list_instances(
        extension_id=EF1_EXTENSION_ID
    )
    full_configs = []
...
def push(
    dt_url: Annotated[str, typer.Option(envvar="DT_URL")],
    dt_token: Annotated[str, typer.Option(envvar="DT_TOKEN")],
    input_file: Annotated[str, typer.Option(help="The location of a previously pulled/exported list of EF1 endpoints")],
    sheet: Annotated[str, typer.Option(help="The name of a sheet in a previously pulled/exported list of EF1 endpoints")],
    ag_group: Annotated[str, typer.Option()],
    version: Annotated[str, typer.Option(help="The version of the EF2 extension you would look to create this configuration for")],
    skip_endpoint_auth: Annotated[bool, typer.Option()] = False,
    enabled: Annotated[bool, typer.Option()] = False,
    merge_commands: Annotated[bool, typer.Option(help="Attempt to combine multiple commands against the same host into one endpoint (based on 'hostname' field)")] = False,
):
    """
    ...
    if not ag_group.startswith("ag_group-"):
        print(f"Appending 'ag_group-' to provided hostname. Result: 'ag_group-{ag_group}'")
    ...
    try:
        result = dt.extensions_v2.post_monitoring_configurations(EF2_EXTENSION_ID, [config])[0]
        print(f"Configs created successfully. Response: {result['code']}")
        base_url = dt_url if not dt_url.endswith("/") else dt_url[:-1]
        print(f"Link to monitoring configuration: {base_url}/ui/hub/ext/listing/registered/{EF2_EXTENSION_ID}/{result['objectId']}/read")
