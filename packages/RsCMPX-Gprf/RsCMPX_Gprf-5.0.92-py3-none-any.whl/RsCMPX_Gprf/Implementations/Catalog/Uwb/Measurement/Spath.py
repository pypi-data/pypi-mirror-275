...
class SpathCls:
	...
	def get(self, stream=repcap.Stream.Default) -> List[str]:
		...
		Snippet: value: List[str] = driver.catalog.uwb.measurement.spath.get(stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_str(f'CATalog:UWB:MEASurement<Instance>:SPATh{stream_cmd_val}?')
