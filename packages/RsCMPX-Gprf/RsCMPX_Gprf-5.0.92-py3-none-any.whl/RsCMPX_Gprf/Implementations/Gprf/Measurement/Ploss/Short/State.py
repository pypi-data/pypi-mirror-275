...
class StateCls:
	...
	def fetch(self, connection_name: str) -> enums.PathLossState:
		"""SCPI: FETCh:GPRF:MEASurement<instance>:PLOSs:SHORt:STATe \n
		Snippet: value: enums.PathLossState = driver.gprf.measurement.ploss.short.state.fetch(connection_name = 'abc') \n
		Queries the result state for the measurement step Short and a selected RF connection. For possible connection names, see
		...
		response = self._core.io.query_str_suppressed(f'FETCh:GPRF:MEASurement<Instance>:PLOSs:SHORt:STATe? {param}', suppressed)
