...
class StateCls:
	...
	def fetch(self, connection_name: str) -> enums.PathLossState:
		...
		Snippet: value: enums.PathLossState = driver.gprf.measurement.ploss.evaluate.state.fetch(connection_name = 'abc') \n
		Queries the result state for the measurement step Evaluation and a selected RF connection. For possible connection names,
		see method RsCMPX_Gprf.Catalog.Gprf.Measurement.Ploss.cname. \n
		...
		response = self._core.io.query_str_suppressed(f'FETCh:GPRF:MEASurement<Instance>:PLOSs:EVALuate:STATe? {param}', suppressed)
