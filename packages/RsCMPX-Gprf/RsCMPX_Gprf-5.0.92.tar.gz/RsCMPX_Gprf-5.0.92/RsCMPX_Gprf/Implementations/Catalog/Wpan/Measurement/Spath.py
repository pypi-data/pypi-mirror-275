...
class SpathCls:
	...
	def get(self, stream=repcap.Stream.Default) -> List[str]:
		...
		Snippet: value: List[str] = driver.catalog.wpan.measurement.spath.get(stream = repcap.Stream.Default) \n
		...
		response = self._core.io.query_str(f'CATalog:WPAN:MEASurement<Instance>:SPATh{stream_cmd_val}?')
