...
class ShortCls:
	...
	def initiate(self) -> None:
		...
		Snippet: driver.gprf.measurement.ploss.short.initiate() \n
		Selects a measurement step according to the last mnemonic (Open, Short or Evaluation) and starts the measurement. \n
		"""
		self._core.io.write(f'INITiate:GPRF:MEASurement<Instance>:PLOSs:SHORt')

	def initiate_with_opc(self, opc_timeout_ms: int = -1) -> None:
		"""SCPI: INITiate:GPRF:MEASurement<instance>:PLOSs:SHORt \n
		Snippet: driver.gprf.measurement.ploss.short.initiate_with_opc() \n
		Selects a measurement step according to the last mnemonic (Open, Short or Evaluation) and starts the measurement. \n
		...
		self._core.io.write_with_opc(f'INITiate:GPRF:MEASurement<Instance>:PLOSs:SHORt', opc_timeout_ms)
