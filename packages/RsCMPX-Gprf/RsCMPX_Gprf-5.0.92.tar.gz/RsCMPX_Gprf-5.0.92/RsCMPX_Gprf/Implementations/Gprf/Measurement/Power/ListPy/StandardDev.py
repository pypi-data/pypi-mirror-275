...
class StandardDevCls:
	...
	def calculate(self, list_index: int) -> List[enums.ResultStatus2]:
		...
		Snippet: value: List[enums.ResultStatus2] = driver.gprf.measurement.power.listPy.standardDev.calculate(list_index = 1) \n
		...
		Suppressed linked return values: reliability \n
			...
			:return: power_std_dev_cur: Power value for the selected segment"""
	...
	def fetch(self, list_index: int) -> List[float]:
		...
		Snippet: value: List[float] = driver.gprf.measurement.power.listPy.standardDev.fetch(list_index = 1) \n
		Returns power results for segment <ListIndex>, see 'Results in list mode'.
			INTRO_CMD_HELP: The following powers can be retrieved: \n
			- Current RMS (...:LIST:CURRent?)
			- Current Min. (...:MINimum:CURRent?)
	...
	def read(self, list_index: int) -> List[float]:
		...
		Snippet: value: List[float] = driver.gprf.measurement.power.listPy.standardDev.read(list_index = 1) \n
		Returns power results for segment <ListIndex>, see 'Results in list mode'.
			INTRO_CMD_HELP: The following powers can be retrieved: \n
			- Current RMS (...:LIST:CURRent?)
			- Current Min. (...:MINimum:CURRent?)
