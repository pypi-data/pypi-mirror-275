...
class GainCls:
	...
	def fetch(self, connection_name: str) -> List[float]:
		...
		Snippet: value: List[float] = driver.gprf.measurement.ploss.evaluate.gain.fetch(connection_name = 'abc') \n
		Returns the gain values of the result table for a selected RF connection. For possible connection names, see method
		...
		response = self._core.io.query_bin_or_ascii_float_list_suppressed(f'FETCh:GPRF:MEASurement<Instance>:PLOSs:EVALuate:GAIN? {param}', suppressed)
		return response
