...
ALL_UNITS = json.loads((Path(__file__).parent.parent.parent / 'data' / 'fm_units.json').open().read())
...
class Dat:
    ...
    def unit(self, id_: str, default: typing.Any = None) -> typing.Union[Handler, list[Handler]]:
    ...
    def link(self, id_: int) -> Link:
        return self.links[id_ - 1]
    ...
    def find_units(self, handler: typing.Union[str, Handler.__class__] = None, sub_type = '') -> list[Handler]:
        ...
        for unit in self._handler2loaded[handler]:
    ...
    def _load_unit(self, fo: TextIO) -> None:
        for line in fo:
            ...
            if unit:
                ...
                self.add_unit(unit)
                if unit.TYPE not in ['boundry', 'hydrology']:
                    self._cur_prog += 1
