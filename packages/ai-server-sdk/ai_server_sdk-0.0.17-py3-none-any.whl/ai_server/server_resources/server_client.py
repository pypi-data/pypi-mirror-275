...
class ServerClient():
        ...
        if 'errorMessage' in json_response:
            if json_response['errorMessage'] == "null principal":
                raise AuthenticationError(
                    url + f" USERID = INVALID could not login using user access key '{self.access_key}' with invalid secret key")
    ...
    def get_partial_responses(self, job_id: str) -> Generator:
        ...
        while True:
            response = requests.post(
                partial_endpoint, cookies=cookies, data=payload).json()
    ...
    def send_request(
        ...
        output_payload_message = self.run_pixel(
            payload="RemoteEngineRun(payload=\"<e>" +
            input_payload_message + "</e>\");",
    ...
    def import_data_product(
        ...
        '''
        base_url = self.main_url + '/project-' + project_id + \
            '/jdbc_json?insightId=' + insight_id + '&open=true&sql='
