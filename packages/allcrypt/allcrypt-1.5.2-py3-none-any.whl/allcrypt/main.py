...
def main():
    ...
    def file_selection_window(mode):
        ...
        progress_bar = ttk.Progressbar(file_selection_window, variable=progress_var, maximum=100)
        ...
        perform_button = ttk.Button(file_selection_window, text=f"{mode} File",
                                    command=lambda: perform_file_operation(mode, input_path_entry.get(), output_path_entry.get(),
                                                                           compress_var.get(), shred_var.get(), progress_var))
        ...
        perform_button.grid(row=4, column=0, pady=10, padx=10, sticky="ew")
    ...
    def perform_file_operation(mode, input_path, output_path, compress, shred, progress_var):
        try:
            ...
            if mode == "Encrypt":
                ...
                write_to_output(output_path, encrypted_data, compress, progress_var)
                if shred:
                ...
                write_to_output(output_path, decrypted_data, False, progress_var)
                if shred:
                    shred_file(input_path)
    ...
    def write_to_output(output_path, data, compress, progress_var):
        total_size = len(data)
            ...
            if compress:
                with gzip.open(output_file, 'wb') as compressed_file:
                    for i in range(0, total_size, chunk_size):
                        compressed_file.write(data[i:i+chunk_size])
                        progress_var.set((i + chunk_size) / total_size * 100)
                        root.update_idletasks()
            else:
                for i in range(0, total_size, chunk_size):
                    output_file.write(data[i:i+chunk_size])
                    progress_var.set((i + chunk_size) / total_size * 100)
                    root.update_idletasks()
    ...
    def decrypt_message():
        try:
            ...
            result_text.config(state="normal")
            result_text.delete("1.0", "end")
            result_text.insert("1.0", "Decryption failed. Check that the USB drive is correct / has the correct key (authorised to decrypt message/file)")
            result_text.config(state="disabled")
    ...
    def get_usb_drive():
        if sys.platform.startswith('win'):
            ...
            raise NotImplementedError("USB drive detection is not supported on this platform.")
    ...
    def encrypt_key_with_password(usb_path, key, password):
        ...
        try:
            ...
            ctypes.windll.kernel32.SetFileAttributesW(os.path.join(usb_path, "encrypted_key.key"), 2 | 4)
        except Exception as e:
            result_text.delete('1.0', 'end')
            result_text.insert("1.0", f"File not found on windows file system - {e}")
    ...
    style = ttk.Style()
    style.configure("TButton", padding=(10, 5), font=('Helvetica', 10), background='lightblue')
