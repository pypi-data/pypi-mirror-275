...
from workspace_clients.models import (
    AssetType,
    ContainerModel,
    AssetModel,
    AssetEmbed,
...
class E360Context(ClientStore):
    ...
    def get_workspace_containers(
        self, name: str = None, include_folders: bool = False
    ) -> List[ContainerAssetModel]:
        ws_client = self.get_workspace_container_client()
        models = []
        location_search_type = (
            LocationSearchType.SUBTREE
            if include_folders
            else LocationSearchType.SINGLE_LEVEL
        )
        for container in ws_client.get(
            name=name, location_search_type=location_search_type
        ):
    ...
    def download_adt_report(
        ...
        if isinstance(asset, str):
            ...
            asset_model = ws_client.get_by_id(asset, params=params)
        else:
            asset_model = asset
    ...
    def download_adt_report_by_id(
        ...
        accept_types = {accept_type[1] for accept_type in ADT_TYPES.values()}
        if content_type not in accept_types:
            raise ValueError(f"The content type needs to be one of {accept_types}")
