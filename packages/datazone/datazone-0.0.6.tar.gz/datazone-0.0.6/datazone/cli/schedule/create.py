...
def create():
    ...
    if job_type == JobType.EXTRACT:
        ...
        CrudServiceCaller(service_name="job", entity_name="extract").get_entity_with_id(entity_id=extract_id)
        payload["extract"] = extract_id
    else:
        pipeline_id = typer.prompt("Pipeline ID", type=str)
        CrudServiceCaller(service_name="job", entity_name="pipeline").get_entity_with_id(entity_id=pipeline_id)
    ...
    CrudServiceCaller(service_name="job", entity_name="schedule").create_entity(payload=payload)
