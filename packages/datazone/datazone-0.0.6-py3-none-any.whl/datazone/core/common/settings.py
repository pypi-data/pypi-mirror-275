...
from datazone.errors.common import DatazoneProfileNotFoundError, DatazoneNotLoggedInError
...
class SettingsManager:
    ...
    def get_profile(cls) -> Profile:
        ...
        profile.password = Fernet(key).decrypt(profile.password.encode()).decode()
        return profile

    @classmethod
    def update_profile_token(cls, token: dict):
        settings = cls.get_settings()
        profile_name = cls.ensure_profile_name()
        token_crypted = Fernet(cls.get_crypto_key()).encrypt(json.dumps(token).encode()).decode()
        settings.profiles[profile_name].token = token_crypted
        settings_file_path = cls.get_settings_path()
        with open(settings_file_path, "w") as file:
            toml.dump(settings.dict(), file)

    @classmethod
    def get_profile_token(cls) -> dict:
        profile = cls.get_profile()
        if profile.token is None:
            raise DatazoneNotLoggedInError
        decrypted_token = Fernet(cls.get_crypto_key()).decrypt(profile.token.encode()).decode()
    ...
    @classmethod
    def create_profile(cls, profile_name: str, username: str, password: str, server_endpoint: str):
        settings = cls.get_settings()
        key = cls.get_crypto_key()
        encrypted_password = Fernet(key).encrypt(password.encode()).decode()
        profile = Profile(username=username, password=encrypted_password, server_endpoint=server_endpoint)
