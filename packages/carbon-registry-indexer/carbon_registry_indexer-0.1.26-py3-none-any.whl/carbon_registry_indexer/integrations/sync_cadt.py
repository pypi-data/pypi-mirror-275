...
def cadt_projects_upsert(df, data_dir, table_name):
    ...
    for index, row in df_cleaned.iterrows():
        ...
        if _id in df_cleaned['cmhq_project_id'] and row['origin_project_id'] != row['project_id']:
            _id = row['current_registry'] + '-' + row['origin_project_id']
...
def cadt_units_upsert(df, data_dir, table_name, issuances_file_name, schema):
    ...
    additional_rows = []
    for cadt_unit_id in retired_df['cadt_unit_id'].unique():
        if cadt_unit_id not in issued_df['cadt_unit_id'].values:
            # Get a template row from the retired dataframe
            template_row = retired_df[retired_df['cadt_unit_id'] == cadt_unit_id].iloc[0].copy()
            template_row['unit_status'] = 'Issued'
            additional_rows.append(template_row)
    
    if additional_rows:
        df_cleaned = pd.concat([df_cleaned, pd.DataFrame(additional_rows)], ignore_index=True)
