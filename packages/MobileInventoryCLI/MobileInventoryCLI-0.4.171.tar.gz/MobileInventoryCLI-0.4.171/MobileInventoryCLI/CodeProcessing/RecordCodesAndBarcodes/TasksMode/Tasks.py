                        ...
                        if str(f) == 'Price':
                            def lclf(text,data):
                                try:
                                    ...
                                    return float(0)
                            dtmp=Prompt.__init2__(None,func=lclf,ptext=f"Entry[default:{data[f]}] {f}",helpText="Enter a value for {f}, or leave blank to use scanned code; 'b' goes back to 'TaskMode'; 'e' to skip/exit entry altogether! 'p' for previous ; 'd' to use default stored value, if you entered a value, then 'd' will use that value when coming back from 'p'",data=self)
                            ...
                            def lcli(text,data):
                                try:
                                    ...
                                    return int(1)
                            dtmp=Prompt.__init2__(None,func=lcli,ptext=f"Entry[default:{data[f]}] {f}",helpText="Enter a value for {f}, or leave blank to use scanned code; 'b' goes back to 'TaskMode'; 'e' to skip/exit entry altogether! 'p' for previous ; 'd' to use default stored value, if you entered a value, then 'd' will use that value when coming back from 'p'",data=self)
                        else:
                            def lclt(text,data):
                                return text
                            dtmp=Prompt.__init2__(None,func=lclt,ptext=f"Entry[default:{data[f]}] {f}",helpText="Enter a value for {f}, or leave blank to use scanned code; 'b' goes back to 'TaskMode'; 'e' to skip/exit entry altogether! 'p' for previous ; 'd' to use default stored value, if you entered a value, then 'd' will use that value when coming back from 'p'",data=self)
...
{Fore.cyan}Name{Fore.light_steel_blue}={Name}{Style.reset}|{Fore.dark_goldenrod}Barcode={Fore.light_green}{BCD}{Style.reset}|{Style.bold}{Fore.yellow}Code={Fore.spring_green_3a}{CD}|{Fore.light_sea_green}ALT_Barcode={Fore.turquoise_4}{ABCD}{Style.reset}
