                                                ...
                                                while True:
                                                    try:
                                                        if str(f) == 'Price':
                                                            ...
                                                            def lcl(text,data):
                                                                return text
                                                        dtmp=Prompt.__init2__(None,func=lcl,ptext=f"Entry {f}",helpText="Enter a value for {f}, or leave blank to use scanned code; e to skip entry altogether! p for previous",data=self)
                                                        if dtmp in ['',None] and f not in ['Price','CaseCount']:
                                                            data[f]=code
                                                        if dtmp in ['',None] and f in ['Price','CaseCount']:
                                                            continue
                                                        if dtmp.lower() in ['e',]:
                                                            return
                                                        if dtmp.lower() in ['p']:
                                                            skipTo=[num for i in enumerate(data)][num-1][0]
                                                            break
                                                        data[f]=dtmp
                                                        break
                                                    except Exception as e:
