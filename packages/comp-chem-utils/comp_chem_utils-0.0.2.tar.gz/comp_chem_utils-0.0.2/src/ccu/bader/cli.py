...
@click.option(
    ...
    "charges for each tag group in the format 'TAG: BADER_CHARGE' where "
    "'TAG' is an integer used to tag atoms in an Atoms object and "
    "'BADER_CHARGE' is the sum of the Bader charge of all atoms with that "
    "tag. To use this option, you must specify a file from which an Atoms "
    "object can be read via ase.io.read via the --atoms option. Note that "
    "if both --smart-mode and INDICES are specified, this option is ignored.",
)
@click.option(
    "--sort-file",
...
def sum_bader(
    ...
    """Sum the bader charges for a collection of atoms specified by
    index.

    Args:
        indices: A list of integers indicating the indices for which to sum
