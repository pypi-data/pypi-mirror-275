...
def archive_results(
    ...
    *,
    ...
    """
    logger.info(f"Archiving results in: {data_file}")
    logger.debug(f"grid: {', '.join(str(x) for x in grid)}")
    logger.debug(f"X_ij0: {', '.join(str(x) for x in chi_ij0)}")
    logger.debug(f"X_ij: {', '.join(str(x) for x in chi_ij)}")
    ...
    logger.info(f"Successfully archived results in: {data_file}")
...
def get_hubbard_u(
    ...
    """Calculates the Hubbard U parameter of a specified atom according to
    the linear response method outlined by M. Cococcioni and S. de
    Gironcoli, Phys. Rev. B 71, 035105 (2005).
    ...
    Args:
        ...
        grid: An iterable of integers specifying the shifting potentials (in
            ...
            Defaults to [-0.2, -0.15, -0.1, -0.05, 0, 0.05, 0.1, 0.15, 0.2].
    ...
    Examples:
        ...
        in NiO similar to the tutorial here:
        ...
        >>> get_hubbard_u(calc, atoms, index, perturbations)  # doctest:+ELLIPSIS
            5....
    """
    ...
    logger.info(f"X_ij0: {chi_ij0_fit[1]}")
    logger.info(f"X_ij: {chi_ij_fit[1]}")
    logger.info(f"Hubbard U calculated for atom {index}: {hubbard_u} eV")
...
def _verify_hubbard_parameters(calc: Vasp):
    ...
    # LORBIT >= 11
    if calc.int_params["lorbit"] is None or calc.int_params["lorbit"] < 11:
        params["lorbit"] = 11
    else:
        params["lorbit"] = calc.int_params["lorbit"]

    # LMAXMIX >= 4
    if calc.int_params["lmaxmix"] is None or calc.int_params["lmaxmix"] < 4:
