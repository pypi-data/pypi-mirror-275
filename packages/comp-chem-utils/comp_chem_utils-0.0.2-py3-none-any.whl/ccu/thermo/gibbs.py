...
def select_frequencies(
    ...
    Args:
        geometry: A string indicating the geometry. One of "linear" or "nonlinear".
            Defaults to "nonlinear".
        approximation: A string indicating the approximation used in the treatment of
            the free energy. Defaults to "HARMONIC".
        transition_state: Whether the frequencies correspond to a transition state.
            Defaults to False.
        frequency_threshold: All frequencies below this value will be normalized to
            this value. Defaults to 12 cm^-1.
...
def calculate_free_energy(
    ...
    """Calculate the quantity Delta G + ZPVE in the specified approximation.
    ...
    Args:
        ...
        geometry: The geometry of the system. One of "linear" or "nonlinear"
            Defaults to "nonlinear".
        ...
        temperature: The temperature for the system. Note that this is only
            relevant if `approximation="IDEAL_GAS"`. Defaults to
            DEFAULT_TEMPERATURE.
        pressure: The pressure for the system. Note that this is only relevant
            if `approximation="IDEAL_GAS"`. Defaults to DEFAULT_PRESSURE.
    ...
    Returns:
        A 3-tuple (ts, zpve, freq) where `ts` is the entropic
        correction, `zpve` is the zero-point vibrational energy,
        and `freq` is a list of floats representing the
        frequencies used to calculate the free energy.
    """
    ...
    zpve = thermo.get_ZPE_correction()
    ts = free_energy - zpve
