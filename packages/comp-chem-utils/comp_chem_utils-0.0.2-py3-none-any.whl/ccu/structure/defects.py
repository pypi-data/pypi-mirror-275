...
def _validate_occupancies(
    *, sites: list[int], occupancies: list[tuple[str, int]]
    ...
    """Verify that the number of occupancies does not exceed the number of
    sites"""
    num_sites = len(sites)
...
def permute(
    *,
    ...
    Args:
        structure: an `ase.Atoms` object to permutate.
    ...
    Example 1: Permute all atoms in a structure

    >>> from ase import Atoms
    >>> from ase.build import bulk

    >>> from ccu.structure.permuter import permute

    >>> permuted_nios = permute(
    ...     structure=bulk("NiO", "rocksalt", a=0.352) * 2
    ... )

    Example 2: Permute all atoms in a set of sites of a structure
    ...
    Example 3: Create Ni-M bimetallics
    ...
    ...             permute(
    ...                 structure=ni,
    ...                 occupancies=(metal, atom.index+1)
    ...             )
    ...         )
    """
    if sites is None:
