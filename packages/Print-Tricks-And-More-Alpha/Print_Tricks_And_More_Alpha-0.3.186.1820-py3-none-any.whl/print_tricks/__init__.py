...
class pt:
    ...
    @staticmethod
    def easy_imports(*import_paths):
        if len(import_paths) == 1 and (isinstance(import_paths[0], list) or isinstance(import_paths[0], tuple)):
            import_paths = import_paths[0]   # Unpack the single list or tuple argument

        # Quick return if all import_paths are already in sys.path
        elif len(import_paths) == 1:
            if import_paths[0] in pt.easy_imports_import_paths:
                return

        # Quick return if all source_dirs are already in the set
        all_in_import_paths = all(src in pt.easy_imports_import_paths for src in import_paths)
        if all_in_import_paths:
            return

        # Add all import_paths to our set
        for path in import_paths:
            if path not in pt.easy_imports_import_paths:
                pt.easy_imports_import_paths.add(path)
        ...
        for path in import_paths:
            ...
            is_full_path = os.path.isabs(path)
            if is_full_path:
                pt._handle_full_path(path, sys_path)
                return
            ...
            pt._handle_partial_path(path, parent, sys_path)
