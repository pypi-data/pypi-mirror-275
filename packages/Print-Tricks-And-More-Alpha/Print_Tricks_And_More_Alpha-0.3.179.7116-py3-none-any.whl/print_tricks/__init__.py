...
class pt:
    ...
    def _print_easy_imports(source, msg=''):
        ...
        import_str = f"{C.t1}  > {'successful' if str(source) in sys.path else 'Did not complete: '}{C.er}" \
                    f" - The directory {C.t2}'{source}'{C.er} {'is now' if str(source) in sys.path else 'does not exist and was not'} added to the system path.{C.er}"
    ...
    @staticmethod
    def easy_imports(*sources):
        if len(sources) == 1 and (isinstance(sources[0], list) or isinstance(sources[0], tuple)):
            sources = sources[0]   # Unpack the single list or tuple argument

        # Quick return if all sources are already in sys.path
        elif len(sources) == 1:
            if sources[0] in pt.easy_imports_sources:
                return

        # Quick return if all source_dirs are already in the set
        all_in_sources = all(src in pt.easy_imports_sources for src in sources)
        if all_in_sources:
            return

        # Add all sources to our set
        for src in sources:
            if src not in pt.easy_imports_sources:
                pt.easy_imports_sources.add(src)

        # Empty source passed
        if not sources:
            pt._handle_empty_source()
            return

        # If one or more sources
        sys_path = sys.path
        ...
        for source in sources:
            ...
            is_full_path = os.path.isabs(source)
            if is_full_path:
                pt._handle_full_path(source, sys_path)
                return

            current_file, file_path, filename, parent, _main_file_path = pt._get_file_info()

            if file_path != _main_file_path:
                return

            pt._handle_partial_path(source, parent, sys_path)

    @staticmethod
    def _handle_empty_source():
        cwd = os.getcwd()
    ...
    @staticmethod
    def _handle_full_path(source, sys_path):
        if os.path.exists(source):
            ...
            pt._print_easy_imports(source=source, msg=f"The path {source} does not exist.")
    ...
    @staticmethod
    def _handle_partial_path(source, parent, sys_path):
        similar_paths = []
        root_reached = False
        possible_mistyped_path = None
        while parent:
            ...
            if source in list_dir_parent:
                ...
                similar_paths = pt._check_similar_paths(source, list_dir_parent, parent, similar_paths)
            if root_reached:
                pt._handle_root_reached(source, similar_paths)
                return
            parent = parent.parent
    ...
    @staticmethod
    def _check_similar_paths(source, list_dir_parent, parent, similar_paths):
        for dir in list_dir_parent:
            ratio = difflib.SequenceMatcher(None, source.lower(), dir.lower()).ratio()
            if ratio > 0.44:
                possible_mistyped_path = parent / dir
    ...
    @staticmethod
    def _handle_root_reached(source, similar_paths):
        if similar_paths:
            similar_paths.sort(key=lambda x: x[2], reverse=True)
            combined_message = "\n".join([
                f"{C.t2}{dir}{C.er} - '{path}'  ({C.t1}{score:.0%}{C.er} similar)"
            ...
            pt._print_easy_imports(source=source, msg=
                f'Did not find "{C.t2}{source}{C.er}", but found similar path(s):\n'
                f'Did you mean to type in one of these?\n\n'
                f'{combined_message}\n')
        else:

            pt.new_trace_level -= 2
            pt._print_easy_imports(source=source, msg="That doesn't seem to exist, ")
