...
class Frame(ctk.CTkFrame):
    ...
    def ext_pages_importer(self, module_name, directory=None, reload=False):
        in_directory = self.U_Pages_dir if directory is None else directory
        reldotpath = in_directory.replace("/", ".").replace("\\", ".")
        if reload:
            mm = importlib.reload(importlib.import_module(f'{reldotpath}.{module_name}'))
        else:
            mm = importlib.import_module(f'{reldotpath}.{module_name}', ".")
        # print(f"imported {module_name}: {mm}")
        try:            
            globals()[module_name] = getattr(mm, module_name)   # import the class, and save it in the globals() so it can be used later
            # print(eval(module_name))
    ...
    def reload_page(self, name):
        if name in self.mainpages_dict:
            self.ext_pages_importer(name, reload=True)

            self.mainpages_dict[name] = eval(name + "()")
            if self.mainpages_dict[name].scrollable:
                Last_widget = ctk.CTkFrame(self.mainpages_dict[name].Scrollable_frame, fg_color=(hvr_clr_g(LIGHT_MODE["background"], "l"), hvr_clr_g(DARK_MODE["background"], "d")), bg_color=(hvr_clr_g(LIGHT_MODE["background"], "l"), hvr_clr_g(DARK_MODE["background"], "d"))) #created to locate the y position for the edge of the frame
                Last_widget.pack(fill="x", pady=10) 
            ...
            self.ext_pages_importer(class_name, subpage_dir, reload=True)

            self.subpages_dict[name] = eval(class_name + "()")
            if self.subpages_dict[name].scrollable:
                Last_widget = ctk.CTkFrame(self.subpages_dict[name].Scrollable_frame, fg_color=(hvr_clr_g(LIGHT_MODE["background"], "l"), hvr_clr_g(DARK_MODE["background"], "d")), bg_color=(hvr_clr_g(LIGHT_MODE["background"], "l"), hvr_clr_g(DARK_MODE["background"], "d"))) #created to locate the y position for the edge of the frame
                Last_widget.pack(fill="x", pady=10) 
