...
def _preprocessing(attention_map, mask, attention_threshold):
    ...
    attention_map = medcam_utils.normalize(attention_map.astype(float))
...
def _intersection_over_attention(binary_attention_map, mask, weights):
    ...
    if weights is not None:
        intersection = intersection.astype(float) * weights
        binary_attention_map = binary_attention_map.astype(float) * weights
...
def _intersection_over_union(binary_attention_map, mask, weights):  # TODO: wiou is bad and wrong, maybe not even possible?
    ...
    if weights is not None:
        ...
        outer_attention = outer_attention.astype(float) * weights
        union = outer_attention + mask.astype(float)
        intersection = intersection.astype(float) * weights
    else:
        union = binary_attention_map | mask
    iou = np.sum(intersection) / np.sum(union).astype(float)
    return iou
