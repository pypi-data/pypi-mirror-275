...
class Programmer:
    def __init__(self):
        ...
        try:
            ...
            if str(ex).startswith('[Errno 13]') and sys.platform == 'Linux':
...
class Interfacetk:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("CH55x Programmer")
    ...
    def create_buttons(self):
        ttk.Button(self.root, text="Connect", command=self.connect_and_detect).pack(pady=5)
        self.erase_button = ttk.Button(self.root, text="Erase Chip", command=self.erase_chip, state="disabled")
        self.erase_button.pack(pady=5)
        self.flash_button = ttk.Button(self.root, text="Flash Firmware", command=self.flash_firmware, state="disabled")
        self.flash_button.pack(pady=5)
        self.verify_button = ttk.Button(self.root, text="Verify Firmware", command=self.verify_firmware, state="disabled")
        self.verify_button.pack(pady=5)
        ttk.Button(self.root, text="Exit Program", command=self.exit_program).pack(pady=5)
        ttk.Button(self.root, text="Run Programmer Exit", command=self.run_programmer_exit).pack(pady=5)

    def create_version_label(self):
        ttk.Label(self.root, text="UNITloader Version: 0.1", font=("Helvetica", 7)).pack(pady=5)
