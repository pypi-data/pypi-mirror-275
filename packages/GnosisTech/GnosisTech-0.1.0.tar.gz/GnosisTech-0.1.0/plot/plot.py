...
def DBScan(data, window_size=10, eps=0.5, min_samples=5, figure_size=(10, 8)):    
    """
    Sử dụng DBSCAN để tìm các outliers trong data.

    Parameters:
        data: pandas Series hoặc DataFrame, shape (n_samples). Input là giá close hoặc return.
        window_size: int, tùy chọn (mặc định = 10). Kích thước của sliding window để tìm các vector đặc trưng.
        eps: float, tùy chọn (mặc định = 0.5). Khoảng cách lớn nhất giữa 2 mẫu để có thể coi chúng là trong cùng một nhóm.
        min_samples: int, tùy chọn (mặc định = 5). Số lượng mẫu nhỏ nhất (hay tổng weight) trong một nhóm để một điểm có thể được chọn là một điểm gốc.
        figure_size: tuple, tùy chọn (mặc định = (10, 8)). Kích thước của hình.
    ...
    # Tạo các vector đặc trưng sử dụng sliding window
    feature_vectors = np.vstack([data[i:i+window_size] for i in range(len(data)-window_size)])
    
    # Normalize các vector đặc trưng
    scaler = StandardScaler()
    feature_vectors_scaled = scaler.fit_transform(feature_vectors)
    
    # Cài đặt các tham số cho DBSCAN và fit vào model
    ...
    ax.plot(data, label='Time Series Data')
    ax.scatter(data.index[outliers_indices], data.iloc[outliers_indices], c='red', label='Outliers')
    ax.set_ylabel('Value')
