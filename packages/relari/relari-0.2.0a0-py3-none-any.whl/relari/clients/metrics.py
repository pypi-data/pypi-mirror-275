import json
from typing import Any, Dict, List, Union
from relari.core.types import HTTPMethod

from relari.core.types import DatasetDatum
from relari.metrics import Metric

ComputeArgs = Union[DatasetDatum, Dict[str, Any]]


class MetricsClient:
    def __init__(self, client) -> None:
        self._client = client

    def compute(
        self, metric: Metric, args: Union[ComputeArgs, List[ComputeArgs]], **kwargs
    ):
        if not isinstance(metric, Metric):
            raise ValueError("metric must be a Metric")
        if not isinstance(args, (DatasetDatum, Dict, list)):
            raise ValueError("args must be a DatasetDatum, Dict, or list of DatasetDatum or Dict")
        if isinstance(args, list):
            return self._batch_compute(metric.value, args, **kwargs)
        else:
            return self._compute(metric.value, args, **kwargs)

    def _compute(self, metric_name: str, args: ComputeArgs, **kwargs):
        # Version 1: metric_name, dataset, and sample_id
        if "dataset" in kwargs and isinstance(args, DatasetDatum):
            endpoint = "metrics/dataset/"
            payload = {
                "metric": metric_name,
                "dataset": kwargs["dataset"],
                "uid": args.label,
                "kwargs": args.data,
            }
        # Version 2: metric_name and args
        elif isinstance(args, Dict):
            endpoint = "metrics/"
            payload = {
                "metric": metric_name,
                "kwargs": args,
            }
        else:
            raise ValueError("Invalid arguments provided to run_metric.")
        response = self._client._request(
            endpoint, HTTPMethod.POST, data=json.dumps(payload)
        )
        if response.status_code != 200:
            raise Exception("Failed to run metric: " + response.text)
        return response.json()

    def _batch_compute(
        self,
        metric_name: str,
        args: List[ComputeArgs],
        **kwargs,
    ):
        if not isinstance(args, list):
            raise ValueError("args must be a list of DatasetDatum or Dict")
        num_datums = len(args)
        if num_datums == 0:
            raise ValueError("args must contain at least one DatasetDatum or Dict")
        if "LLMBased" in metric_name and self._client.timeout < 4*num_datums:
            raise ValueError(f"This is a synchronous metric, please increase the timeout for LLM based metrics (suggested {6*num_datums}) or reduce batch size.")
        # Mode 1: DatasetDatum
        if "dataset" in kwargs and isinstance(args[0], DatasetDatum):
            endpoint = "metrics/dataset/batch/"
            payload = {
                "metric": metric_name,
                "dataset": kwargs["dataset"],
                "kwargs": [x.asdict() for x in args],
            }
        else:
            endpoint = "metrics/batch/"
            payload = {
                "metric": metric_name,
                "kwargs": args,
            }
        response = self._client._request(
            endpoint, HTTPMethod.POST, data=json.dumps(payload)
        )
        if response.status_code != 200:
            raise Exception("Failed to run metric: " + response.text)
        return response.json()
