...
def image_detect_image_border(in_file, direction, color_threshold=30, border_threshold=0.1):
    ...
    def scan_pixels(start, step, max_value, axis):
        border_width = 0
        for i in range(start, max_value, step):
            if axis == "horizontal":
                ...
                pixels_list = pixels[i, :, :]
            avg_color_diff = np.mean(np.linalg.norm(pixels_list - pixels_list[0], axis=1))
            if avg_color_diff < color_threshold:
                border_width += 1
            else:
                break
        return border_width
    ...
    if direction == "horizontal":
        left_border_width = scan_pixels(0, 1, image_width, "horizontal")
        right_border_width = scan_pixels(image_width - 1, -1, -1, "horizontal")
        return left_border_width, right_border_width
    elif direction == "vertical":
        top_border_height = scan_pixels(0, 1, image_height, "vertical")
        bottom_border_height = scan_pixels(image_height - 1, -1, -1, "vertical")
