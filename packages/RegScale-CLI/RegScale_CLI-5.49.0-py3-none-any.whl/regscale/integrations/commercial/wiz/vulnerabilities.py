...
class VulnerabilitiesIntegration:
    ...
    def map_node_data(self, nodes: List[Dict]):
        ...
        """
        for node in nodes:
            # node_projects = node.get("projects", [])
            # if not any([project == self.project for project in node_projects]):
            #     continue
            asset = self.asset_dict.get(node.get("vulnerableAsset", {}).get("id"))
            if asset:
                existing_scan = self.find_existing_scan(asset, node)
                scan_history = self.set_existing_scan(existing_scan, asset, node)
    ...
    def find_existing_scan(self, asset: regscale_models.Asset, node: Dict) -> regscale_models.ScanHistory:
        """
        ...
        for scan in existing_scan_histories:
            if scan.scanningTool == self.scan_tool and date_obj(scan.scanDate) == date_obj(node.get("publishedDate")):
                existing_scan = scan
                break
        return existing_scan

    def set_existing_scan(
        self, existing_scan: regscale_models.ScanHistory, asset: regscale_models.Asset, node: Dict
        ...
        if not existing_scan:
            scan_history = regscale_models.ScanHistory(
                ...
                parentId=asset.id,
                parentModule="assets",
                scanDate=node.get("publishedDate"),
            ).create()
        else:
    ...
    def determine_closed_date(node: Dict) -> Optional[str]:
        ...
        :rtype: Optional[str]
        """
        if node.get("status").lower() == "resolved":
            if resolved_date := get_value(node, "resolvedAt"):
                return resolved_date
            else:
                return get_value(node, "lastDetectedAt")
        return None
    ...
    def update_issue(
        ...
        issue.status = self.map_status_to_issue_status(node.get("status"))
        ...
        issue.dateCompleted = self.determine_closed_date(node)
