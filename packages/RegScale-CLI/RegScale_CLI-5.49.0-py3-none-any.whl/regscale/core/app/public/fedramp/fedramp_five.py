...
def get_or_create_option(
    ...
    if not option:
        try:
            ...
            options = ImplementationOption.get_all_by_parent(parent_id=control.id, plan_id=parent_id)
            for o in options:
                if o.name == part_name:
                    option = o
    return option
...
def handle_parts(
    parts: Dict, status: str, control: SecurityControl, control_implementation: ControlImplementation, parent_id: int
):
    """
    ...
    for index, part in enumerate(parts):
        ...
        for objective in matching_objectives:
            ...
            if has_existing_obj:
                continue
            handle_implementation_objectives(objective, option, status, control_implementation, imp_objectives, control)
...
def get_part_letter(part: str) -> str:
    ...
    :rtype: str
    """
    return part[-1].lower()  # Assuming the format "Part X" where X is the part letter.
...
def handle_params(
    ...
    for param in parameters:
        control_param_name = mappings.get(f"Parameter {param.get('name').strip()}")
