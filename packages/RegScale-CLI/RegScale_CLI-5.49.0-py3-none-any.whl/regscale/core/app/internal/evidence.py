...
def start():
    ...
    with progress:
        ...
        global directories
        directories = set_directory_variables(task=task0, evidence_folder=config["evidenceFolder"])
        ...
        # call function to define variable for use outside of function
        global times
        times = get_doc_timestamps(evidence_folder=directories[2], directory=directories[1], task=task2)
        ...
        # call function to define variable for use outside of function
        global folders
        folders = find_required_files_in_folder(evidence_folder=directories[2], task=task4)

        task5 = progress.add_task("[white]Searching for digital signatures in documents...", total=2)

        # call function to define variable for use outside of function
        global SIGRESULTS
        SIGRESULTS = signature_assessment_results(directory=folders, r_docs=docs[0], task=task5)

        task6 = progress.add_task("[white]Testing if required documents are present...", total=2)

        # call function to define variable for use outside of function
        global DOCRESULTS
        DOCRESULTS = document_assessment_results(directory=folders, documents=docs[1], task=task6)

        task7 = progress.add_task("[white]Extracting texts from required files...", total=4)

        # call function to define variable for use outside of function
        global file_texts
        file_texts = parse_required_text_from_files(evidence_folder=directories[2], task=task7)

        task8 = progress.add_task("[white]Searching for required text in parsed documents...", total=2)

        # call function to define variable for use outside of function
        global search_results
        search_results = text_string_search(f_texts=file_texts, req_texts=texts, task=task8)

        task9 = progress.add_task("[white]Testing if required texts are present", total=2)

        # call function to define variable for use outside of function
        global TEXTRESULTS
        TEXTRESULTS = text_assessment_results(searches=search_results, r_texts=texts, task=task9)

        task10 = progress.add_task("[white]Retrieving data from the evidence test projects...", total=3)

        # call function to define variable for use outside of function
        global data
        data = gather_test_project_data(api=api, evidence_folder=directories[0], task=task10)
        ...
        global TIMERESULTS
        TIMERESULTS = assess_doc_timestamps(timestamps=times, documents=docs[0], task=task11)
        ...
        create_child_assessments(api=api, project_data=data, output=html_output, task=task16)
...
def set_directory_variables(task: int, evidence_folder: str) -> Tuple[str, str, str]:
    """
...
def parse_required_docs(evidence_folder: str, task: int) -> Tuple[list[dict], set[str]]:
    """
...
def get_doc_timestamps(evidence_folder: str, directory: str, task: int) -> list[dict]:
    """
...
def set_required_texts(evidence_folder: str, task: int) -> set[str]:
    """
...
def find_required_files_in_folder(evidence_folder: str, task: int) -> list[dict]:
    """
...
def signature_assessment_results(directory: list[dict], r_docs: list[dict], task: int) -> list[dict]:
    """
...
def document_assessment_results(directory: list[dict], documents: set[str], task: int) -> list[dict]:
    """
...
def parse_required_text_from_files(evidence_folder: str, task: int) -> list[dict]:
    """
...
def text_string_search(f_texts: list[dict], req_texts: set[str], task: int) -> list[dict]:
    """
...
def text_assessment_results(searches: list[dict], r_texts: set[str], task: int) -> list[dict]:
    """
...
def gather_test_project_data(api: Api, evidence_folder: str, task: int) -> list[dict]:
    """
...
def assess_doc_timestamps(timestamps: list[dict], documents: list[dict], task: int) -> list[dict]:
    """
...
def build_assessment_dataframe(assessments: list[dict], task: int) -> list[dict]:
    """
...
def build_score_data(assessments: list[dict], task: int) -> Tuple[list[int], list[int], list[int]]:
    """
...
def build_html_table(assessments: list[dict], task: int) -> list[dict]:
    """
...
def create_child_assessments(api: Api, project_data: list[dict], output: list[dict], task: int) -> None:
    """
