            ...
            for expr in scope_expr.expressions:
                for column in expr.find_all(exp.Column):
                    ...
                    if iteration_count > max_iterations:
                        raise Exception(
                            "The number of loops exceeds the maximum limit and an infinite loop may exist."
                        )
                    if (
                        expr.args.get("alias") is not None
                        and expr.args.get("alias").alias_or_name == column.this.alias_or_name
                    ):
                        continue
                    # For example, this type of column, clickhouse's tuple type, like t.1
                    if column.args.get("table"):
