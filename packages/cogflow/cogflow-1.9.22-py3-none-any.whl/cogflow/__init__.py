...
def search_registered_models(
    ...
    """
    return MlflowPlugin().search_registered_models(filter_string=filter_string, max_results=max_results,
...
def register_model(model_uri: str, model: str, await_registration_for: int = 300, *,
...
def create_registered_model(model: str, tags: Optional[Dict[str, Any]] = None,
    ...
    """
    return MlflowPlugin().create_registered_model(model=model, tags=tags,
                                                  description=description)


def create_model_version(model: str, source: str, run_id: Optional[str] = None,
    ...
    """
    return MlflowPlugin().create_model_version(model=model, source=source, run_id=run_id,
                                               tags=tags, run_link=run_link, description=description,
...
def set_experiment(experiment_name: Optional[str] = None, experiment_id: Optional[str] = None):
    """
    ...
    Returns:
        None
    """
    return MlflowPlugin().set_experiment(experiment_name=experiment_name, experiment_id=experiment_id)
...
def start_run(run_id: Optional[str] = None,
    ...
    """
    return MlflowPlugin().start_run(run_id=run_id, experiment_id=experiment_id, run_name=run_name,
...
def link_model_to_dataset(dataset_id, model_id):
    ...
    """
    return MlflowPlugin().link_model_to_dataset(
...
def save_model_details_to_db(registered_model_name):
    ...
    """
    return MlflowPlugin().save_model_details_to_db(
        registered_model_name=registered_model_name
    )
...
def get_model_latest_version(registered_model_name):
    ...
    """
    return MlflowPlugin().get_model_latest_version(
        registered_model_name=registered_model_name
    )
...
def search_model_versions(
    ...
    """
    return MlflowPlugin().search_model_versions(
