...
from .getauth import getASRAIAuth
...
def test_one():
    ...
    asrdata = getASRAIAuth(apikey=apikey, cagetory=cagetory)

    asrdata=asrdata['aiinfo']
    appid, token, cluster = asrdata['appid'], asrdata['token'], asrdata['cluster']  # 项目的 appid \项目的 token\ 请求的集群

    audio_path = "test4.wav"  # 本地音频路径
...
def asr(apikey,audio_path):
    ...
    if asrdata['ret']==0:
        ...
        audio_format = os.path.splitext(audio_path)[1][1:] # wav 或者 mp3，根据实际音频格式设置
