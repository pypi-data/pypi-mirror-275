...
from syncmodels.syncmodels import NonRecoverable, SyncModel, COPY
...
class iBot(iAgent):
    ...
    def _build_params(self, **context):
        ...
        def match(text) -> bool:
            ...
            if text in ("kind",):
                log.warning(
                    "[%s] key is deprecated, use KIND_KEY instead", text
                )
                return False
...
class iSQLSession(iSession):
    ...
    def _locate_data(self, data, rows=1, **kw):
        ...
        async with self as session:
            ...
            async with response:
                if 200 <= response.status < 300:
                    ...
                    kw = {
                        k: result[v]
                        for k, v in self.RESULT_STRUCTURE.items()
                    }
