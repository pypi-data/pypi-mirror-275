...
class iSession(iContext, iSchema, iAuthenticator):
    ...
    async def _get_connection(self, url, **kw):
        ...
        if xhost not in self.connection_pool:
            self.connection_pool[xhost] = await self._create_connection(url, **kw)
    ...
    async def new(cls, url, bot, **context):
        def score(item):
            ...
            uri, factory, m = item
            sc = 10 * len(m.groups()) + len([_ for _ in uri.values() if _ is not None])
            # sc = len(m.groups())
    ...
    async def update_params(self, url, params, context):
        ...
        if wave0:
            ...
            # TODO: use cache
            d_fields, fields, monotonic_since_key = await self._get_schema(uri)
...
class iAgent:
    ...
    async def bootstrap(self):
        ...
        for func, args, kwargs in self._bootstrap():
            ...
            if not wave0:
                log.warning("Can't find initial sync wave for kind: [%s]", kind)
            else:
...
class iSQLSession(iSession):
    ...
    async def _inspect_schema(self, uri, kind):
        ...
        async with self as session:
            ...
            response = await method(url, headers=self.bot.headers, json=payload)
            async with response:
                if 200 <= response.status < 300:
                    result = await response.json()
                    self.RESULT_STRUCTURE = self._locate_data(result, rows=N)
                    kw = {k: result[v] for k, v in self.RESULT_STRUCTURE.items()}
...
class SQLiteSession(iSQLSession):
    ...
    async def get(self, url, headers=None, params=None, **kw):
        ...
        body = [{ord_fields[i]["name"]: v for i, v in enumerate(row)} for row in res]
