...
class Connection:
    ...
    def _client_handshake_GCM(
        ...
        while True:
            ...
            peer_key = load_pem_public_key(
                rsp1.pubKey.encode(encoding="UTF-8", errors="strict"), backend=default_backend()
            )
    ...
    def _get_authenticators(self, database: str) -> Any:
        ...
        if rsp.response.type != proto.ConfirmationResponse.RESPONSE_OK:
            raise Error(f"Error when fetching authenticators for database {self.database}: {rsp.response.reason}")
...
class Cursor:
    ...
    def executemany(self, operation: str, parameterlist: Iterable[Mapping[str, object]]) -> "Cursor":
    ...
    def execute(self, operation: str, parameters: Mapping[str, object] = {}) -> "Cursor":
    ...
    def _expand_parameters(self, operation: str, parameters: Mapping[str, object]) -> str:
        formatted_parameters: Dict[str, object] = {}
        for key, value in parameters.items():
            if value is None:
                formatted_parameters[key] = "NULL"
            elif isinstance(value, str):
                formatted_parameters[key] = f"'{value}'"
            else:
                formatted_parameters[key] = value
        return operation % formatted_parameters
    ...
    def _execute_internal(self, operation: str, parameters: Mapping[str, object] = {}) -> None:
        ...
        if parameters:
            operation = self._expand_parameters(operation, parameters)
        ...
        while True:
            ...
            try:
                ...
                self.connection = Connection(
                    ...
                    performance_mode=self.connection.performance_mode,
                )
    ...
    def _execute_force_external(self, operation: str) -> None:
        ...
        if rsp.type == proto.ConfirmationResponse.RESPONSE_WARN:
            warn(Warning(rsp.reason, rsp.sql_state, rsp.vendor_code))
