...
def sso_get_credentials_from_code(
    ...
    # the password consains a base64 encoded json document with tokens (likely
...
def sso_get_credentials(
    ...
    raise api.Error("Unable to authenticate")
...
def local_sso_callback(auth_url: str) -> Tuple[str, str]:
    ...
    with ContextHTTPServer(("localhost", LOCAL_SSO_CALLBACK_PORT), SingleRequestHandler) as server:
        ...
        if webbrowser.open_new(auth_url):
            ...
            logger.warn(f"Unable to open web browser for {auth_url}")
