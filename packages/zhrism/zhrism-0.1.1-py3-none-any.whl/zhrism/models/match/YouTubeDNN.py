...
class YouTubeDNN(torch.nn.Module):
    ...
    Args:
        ...
        user_params (dict): the params of the User Tower module, keys include:
    ...
    def forward(self, x):
        ...
        # mul:逐元素相乘
        # （batch_size,1,embed_dim）,(batch_size,1+n_neg_items,embed_dim)
        y = torch.mul(user_embedding, item_embedding).sum(dim=2)
        # 温度系数
        # Reference：Sampling-Bias-Corrected Neural Modeling for Large Corpus Item Recommendations
        y = y / self.temperature
        # （batch_size,1+n_neg_items）
        # Point-wise:n自己指定，独立考虑每个item
        # Pair-wise:n=1
        # List-wise:n自己指定，不同于Point-wise的是，其将正item与多个负item一起视作一个多分类问题
        return y
