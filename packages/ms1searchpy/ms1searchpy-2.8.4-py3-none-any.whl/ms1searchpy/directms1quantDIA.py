...
def get_df_final_DIA(args, replace_label, allowed_peptides, allowed_prots_all, pep_RT=False, RT_threshold=False):

    df_final = False

    diann_pep_table = pd.read_table(args['diann_res'])
    samples_table = pd.read_table(args['samples'])
    samples_map = samples_table.set_index('File Name')['group'].to_dict()

    for z in set(samples_table['group']):
        df3 = diann_pep_table[diann_pep_table['Run'] == z]
        print(z, len(df3))
...
def run():
    ...
    parser.add_argument('-diann_res', help='input DiaNN report file', required=True)
    parser.add_argument('-samples', help='tsv table with sample details', required=True)
    parser.add_argument('-out', help='name of DirectMS1quantDIA output file', default='directms1quantDIA_out')
    ...
    if args['allowed_peptides']:
        ...
        allowed_peptides = set(cnt0.keys())
    ...
    df_out = df_out[['score', 'p-value', 'dbname', 'log2FoldChange(S2/S1)', 'FC_pass', 'FDR_pass', 'differentially expressed peptides',
                    'identified peptides', 'log2FoldChange(S2/S1) no normalization', 'log2FoldChange(S2/S1) using all peptides',
                    'log2FoldChange(S2/S1) using all peptides and no normalization', 'protname', 'protein_quant_group', 'gene', 'BH_threshold']]
