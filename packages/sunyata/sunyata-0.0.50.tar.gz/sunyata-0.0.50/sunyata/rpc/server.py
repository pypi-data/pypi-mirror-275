...
class HttpRpcServer(RpcServer):
    ...
    async def asyncServe(self):
        tasks = []
        tasks.append(asyncio.get_event_loop().create_task(self.app.asyncServe()))
        if self.discovery and self.discoveryConfig:
            registTask = asyncio.get_event_loop().create_task(self.discovery.asyncRegist(self.discoveryConfig.serviceName, self.discoveryConfig.serviceHost, self.discoveryConfig.servicePort, ttlHeartBeat=True))
...
class UdpRpcServer(RpcServer):
    ...
    async def asyncServe(self):
        ...
        tasks = []
        if self.discovery and self.discoveryConfig:
            tasks.append(asyncio.get_event_loop().create_task(self.discovery.asyncRegist(self.discoveryConfig.serviceName, self.discoveryConfig.serviceHost, self.discoveryConfig.servicePort, ttlHeartBeat=True)))
        tasks.append(asyncio.get_event_loop().create_task(self.handlePackage()))
        loop = asyncio.get_event_loop()
        rs = loop.run_until_complete(asyncio.gather(*tasks))
        try:
            loop.run_forever()
