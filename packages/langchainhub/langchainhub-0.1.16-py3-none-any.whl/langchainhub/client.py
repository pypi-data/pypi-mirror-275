    ...
    def push(
        ...
        *,
        ...
        try:
            ...
            repo = self.get_repo(full_repo)
        except requests.exceptions.HTTPError as e:
            if e.response.status_code != 404:
                raise e
            # create repo if it doesn't exist
            # make sure I am owner if owner is specified
            if settings["tenant_handle"] and owner != '-' and settings["tenant_handle"] != owner:
                raise ValueError(
                    f"Tenant {settings['tenant_handle']} is not the owner of repo {repo_full_name}"
                )
            self.create_repo(
                repo_handle, is_public=new_repo_is_public, description=new_repo_description
            )
        ...
        url = self._host_url + f"/prompts/{repo_handle}/{short_hash}?organizationId={settings['id']}"
        return url
