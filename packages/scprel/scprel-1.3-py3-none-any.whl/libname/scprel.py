...
def scrun(names, path):
  ...
  gencode = pd.read_table("gencode.v44.annotation.gff3.gz", comment="#",
                          sep = "\t",
  ...
  print('Genes location annotation...')
  %timeit
  for i in samr:
    adata = i
  ...
  for i in samr:
    ...
    fig.savefig('folfox_Resp_'+str(names[k])+'cnvscore.png', dpi=300,  facecolor ='w')
  ...
  for i in samr:
    ...
    cnv.pl.chromosome_heatmap(adata, dendrogram=True,
                        save = ('folfiri_n'+str(names[k])+'cnvscore.png')
                         )
