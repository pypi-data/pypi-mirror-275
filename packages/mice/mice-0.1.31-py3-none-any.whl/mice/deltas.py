...
class DeltaSparse:
    """Class for low memory deltas in MICE with sparse iterates
    ...
    def __init__(self, x, sampler, c=2, x_l1=None, m_min=5):
        ...
        self.m_prev = 0
        self.sampler = sampler
        self.welford = WelfordSparse(dim=x.shape)
        self.welford_base = WelfordSparse(dim=x.shape)
    ...
    def update_delta(self, mice, m):
        if self.m < m:
            ...
            if self.c == 1:
                new_values = mice.grad(self.x_l, samples)
                self.welford.update(new_values)
                self.welford_base.update(new_values)
            else:
                new_f_ls = mice.grad(self.x_l, samples)
                new_f_l1s = mice.grad(self.x_l1, samples)
                self.welford.update(new_f_ls - new_f_l1s)
                self.welford_base.update(new_f_ls)
            mice.counter += self.c * m_to_sample
        return
    ...
    def restart(self, mice):
        new_delta = mice.create_delta(self.x_l, c=1)
        new_delta.x_l1 = None
        new_delta.welford = self.welford_base
        new_delta.sampler = self.sampler
        new_delta.c = 1
        new_delta.m_prev = self.m_prev
        new_delta.m_min = mice.m_restart_min
    ...
    @property
    def v_l(self):
        return self.welford.s2

    @property
    def v_batch(self):
        return self.welford_base.s2
...
class Welford:
    ...
    def update(self, x):
        ...
        self.mean += 1 / self.count * (np.sum(x, axis=0) - x_len * self.mean)
        self.m2 += np.sum((x - self.mean) ** 2) - (old_mean - self.mean) @ (np.sum(x - self.mean, axis=0))
        self.s2 = self.m2 / (self.count - 1)
...
class WelfordSparse:
    ...
    def update(self, x):
        ...
        old_mean = self.mean.copy()
        self.mean += 1 / self.count * (np.sum(x, axis=0) - x_len * self.mean)
        self.m2 += np.sum(np.square(x - self.mean)) - (old_mean - self.mean) @ (x - self.mean).sum(axis=0)
        self.s2 = self.m2 / (self.count - 1)
