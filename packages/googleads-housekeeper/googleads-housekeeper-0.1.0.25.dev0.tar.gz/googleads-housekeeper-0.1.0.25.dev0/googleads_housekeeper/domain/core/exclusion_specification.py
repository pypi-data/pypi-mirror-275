...
class BaseExclusionSpecificationEntry:
    ...
    def is_satisfied_by(self, placement_info: GaarfRow) -> tuple[bool, dict]:
        ...
        serialized_placement = self._safe_json_dumps(placement_as_dict or placement)
        if not hasattr(placement, self.name):
            if self.name == 'conversion_name':
                return False, {}
            else:
                raise ValueError(
                    f'{serialized_placement} has no {self.name} attribute!')
    ...
    def _safe_json_dumps(self, obj:any) -> str:
        """
        Safely serializes an object to JSON, including only fields that have
        'to_dict' and are JSON serializable, recursively handling nested objects.
        """
        def _to_dict(obj):
            if isinstance(obj, dict):
                return {k: _to_dict(v) for k, v in obj.items() if _is_json_serializable(v)}
            elif hasattr(obj, 'to_dict'):
                try:
                    dict_repr = obj.to_dict()
                    return {
                        k: _to_dict(v) for k, v in dict_repr.items() if _is_json_serializable(v)
                    }
                except Exception:
                    return ''
            elif isinstance(obj, (list, tuple)):
                return [_to_dict(item) for item in obj if _is_json_serializable(item)]
            else:
                return obj if _is_json_serializable(obj) else None  # For basic types
        ...
        if not _is_json_serializable(obj):
            return ''
        return json.dumps(_to_dict(obj), indent=4)
