...
def analisar_independencia_variaveis_tabela_contingencia(tabela_contingencia):
    """
    ...
    if n_linhas == 2 and n_colunas == 2:
        ...
        resultados_df = pd.DataFrame({
            'Value': [res_chi2[0], res_chi2_corr[0], res_likelihood_ratio[0], res_fisher_2sided[0], tabela_contingencia.sum().sum()],
            'df': [res_chi2[2], res_chi2_corr[2], res_likelihood_ratio[2], "", ""],
            'Asymp. Sig. (2-sided)': [res_chi2[1], res_chi2_corr[1], res_likelihood_ratio[1], "", ""],
            'Exact Sig. (2-sided)': ["", "", "", res_fisher_2sided[1], ""],
            'Exact Sig. (1-sided)': ["", "", "", res_fisher_1sided[1], ""],
        }, index=["Pearson Chi-Square", "Continuity Correction", "Likelihood Ratio", "Fisher`s Exact Test", "N. of Valid Cases"])

    # Teste de Qui-Quadrado e Razão de Verossimilhança para qualquer tamanho de tabela
    else:
        # Cálculos
        res_chi2 = chi2_contingency(tabela_contingencia, correction=False)
        res_chi2_corr = chi2_contingency(tabela_contingencia, correction=True)
        ...
        # Criando um DataFrame com os resultados
        resultados_df = pd.DataFrame({
            'Value': [res_chi2[0], res_chi2_corr[0], res_likelihood_ratio[0], tabela_contingencia.sum().sum()],
            'df': [res_chi2[2], res_chi2_corr[2], res_likelihood_ratio[2], ""],
            'Asymp. Sig. (2-sided)': [res_chi2[1], res_chi2_corr[1], res_likelihood_ratio[1], ""],
        }, index=["Pearson Chi-Square", "Continuity Correction", "Likelihood Ratio", "N. of Valid Cases"])
    ...
    resultados_df_formated = resultados_df.applymap(lambda x: '{:.3f}'.format(x) if isinstance(x, (int, float)) else x)
    resultados_df_formated.iloc[:,0] = resultados_df.iloc[:,0].apply(lambda x: '{:.2f}'.format(x) if isinstance(x, (int, float)) else x)
    resultados_df_formated.loc['N. of Valid Cases', 'Value'] = '{:.0f}'.format(resultados_df.loc['N. of Valid Cases', 'Value'])
    resultados_df_formated.iloc[:,1] = resultados_df.iloc[:,1].apply(lambda x: '{:.0f}'.format(x) if isinstance(x, (int, float)) else x)
...
def calcular_odds_ratio_razao_risco_discrepancia(tabela_contingencia, print_results=False):
    ...
    results_df = pd.DataFrame({
        ...
        '95% CI Upper': [odds_ratio_ci[1], risk_ratio_1_ci[1], risk_ratio_2_ci[1], ""]
              ...
              f'RR (ou RP) for {nome_grupo[0]} = {categorias_grupo1[0]}', 
              f'RR (ou RP) for {nome_grupo[0]} = {categorias_grupo1[1]}',
              "N. of Valid Cases"])
...
def complementar_tabela_contingencia_com_analise_estatistica(tabela_contingencia):
    """
    ...
    e resíduos ajustados estandardizados. Também adiciona uma visualização estruturada dos dados
    ...
    A função realiza as seguintes operações na tabela de contingência fornecida:
    - Calcula as frequências esperadas usando o modelo estatístico adequado.
    - Determina as probabilidades condicionadas de um grupo dado o outro.
    - Calcula percentagens totais de contagens em relação ao total geral.
    - Computa resíduos de Pearson (não ajustados) e resíduos ajustados estandardizados para avaliação de desvios.
    - Reestrutura o DataFrame resultante para uma apresentação clara de todas as análises, incluindo a adição de totais.

    O resultado é um DataFrame abrangente que fornece uma visão detalhada das relações e discrepâncias
    dentro dos dados originais, facilitando a interpretação estatística.
    """
    ...
    index_keys = [
        ...
        '% of Total', 'Standardized Residual', 'Adjusted Residual'
    ]
    ...
    combined_df = pd.concat([
        ...
        perc_total * 100,
        residuos_nao_ajustados,
        residuos_estandardizados
    ], axis=0, keys=index_keys)
...
def analisar_frequencias_esperadas_tabela_contingencia_2x2xk(dataframe_multi_nivel):
    """
    ...
    for indices, subgrupo in dataframe_multi_nivel.groupby(level=niveis_para_agrupar):
        ...
        print('Fe:', frequencias_esperadas)
        print('\n', nota_de_rodape)
        print("-" * 100)  # Separador visual para cada análise
        
def backward_elimination_with_comparison_llm(dataframe, display_models=True, compare_models=True):
    """
    ...
    def generate_interactions(predictors):
    ...
    all_predictors = predictors + generate_interactions(predictors)
...
"""    
def backward_elimination(data, models=True, comparation=True):

    import warnings
    # Suprime RuntimeWarnings específicos relacionados a divide by zero
    warnings.filterwarnings("ignore", message="divide by zero encountered in scalar divide")

    # Suprime PerfectSeparationWarning de statsmodels
    from statsmodels.tools.sm_exceptions import PerfectSeparationWarning
    warnings.filterwarnings("ignore", category=PerfectSeparationWarning)
    ...
    # Vamos assumir que as primeiras colunas são as variáveis categóricas e a última coluna é a de frequência
    basic_predictors = data.columns[:-1].tolist()
    response = data.columns[-1]

    # Gera todas as interações possíveis internamente
    def generate_all_possible_interactions(predictors):
        all_interactions = []
        for i in range(2, len(predictors) + 1):
            for combo in itertools.combinations(predictors, i):
                interaction_term = ':'.join(combo)
                all_interactions.append(interaction_term)
        return all_interactions

    # Combina variáveis principais e interações
    initial_predictors = basic_predictors + generate_all_possible_interactions(basic_predictors)
    predictors = initial_predictors[:]
    satured_model_formula = f"{response} ~ {' + '.join(predictors)}"
    # example: formula = "Freq ~ C(Religião) * C(Autonomia) * C(Satisfação)"
    satured_model = glm(formula=satured_model_formula, data=data, family=sm.families.Poisson()).fit()
    adjusted_model_formula = satured_model_formula
    adjusted_model = satured_model

    if models==True:
        print("\nModelo saturado:")
        print(satured_model.summary())
        print("\nAjustando modelo:")

    i = 1
    while True:
        p_values = adjusted_model.pvalues.iloc[1:]  # exclui o intercepto
        max_p_value = p_values.max()

        if max_p_value > 0.05:
            worst_predictor = p_values.idxmax()
            predictors.remove(worst_predictor.split('[')[0])  # Ajuste para variáveis categóricas
            if models==True:
                print(f"Passo {i}: Removendo {worst_predictor} com p-valor {max_p_value:.3f}")
            adjusted_model_formula = f"{response} ~ {' + '.join(predictors)}"
            adjusted_model = glm(formula=adjusted_model_formula, data=data, family=sm.families.Poisson()).fit()
        else:
            break
        i = i + 1
    if models==True:
    ...
    if comparation == True:
        # Compara os modelos usando AIC
        print("\nComparação entre satured_model e adjusted_model:\n")
        print(f"Satured Model AIC: {satured_model.aic:.2f}")
        print(f"Adjusted Model AIC: {adjusted_model.aic:.2f}")
    
        # Realização do Teste de Razão de Verossimilhança (LRT)
        lr_stat = 2 * (satured_model.llf - adjusted_model.llf)
        diff_num_params = len(satured_model.params) - len(adjusted_model.params)
        p_value = chi2.sf(lr_stat, df=diff_num_params)
        
        print("\nTeste de Razão de Verossimilhança (LRT):")
        print(f"LR stat: {lr_stat:.3f}, diferença no número de parâmetros: {diff_num_params}, p-value: {p_value:.3f}")
    
    return satured_model, adjusted_model
"""

def calculate_and_display_odds_ratios_llm(model):
    """
...
def criarTabelaContingenciaExpandida_llm(dados_df, grupos, categorias, model):
    """
...
def realizar_analise_correspondencia(df, n_components=2, focus='row', df_supplement=None):
    """
...
def analise_correspondencia_e_grafico(tabela_contingencia,  x_component=0, y_component=1, tabela_contingencia_suple=None):
    ...
    chart = ca.plot(
        tabela_combinada,
        x_component=0,  # Primeira dimensão para o eixo X
        y_component=2,  # Segunda dimensão para o eixo Y
