...
class BaseDynamodbModel(BaseModel):
    ...
    def convert_to_raw(self, data_dict: dict, root_level: bool = False) -> dict:
        return {item_key: self.convert_to_raw_value(item_value, root_level=root_level) for (item_key, item_value) in data_dict.items()}
    ...
    def convert_to_raw_value(self, item_value: Any, root_level: bool = False):
        match item_value:
            ...
            case dict():
                item_value_dict = {"M": self.convert_to_raw(item_value)}
            case list():
                item_value_dict = {"L": [self.convert_to_raw_value(item_elem) for item_elem in item_value]}
            case str():
        ...
        return item_value_dict if root_level else item_value_dict.values().pop()
