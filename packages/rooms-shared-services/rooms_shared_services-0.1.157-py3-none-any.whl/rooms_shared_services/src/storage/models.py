...
class BaseDynamodbModel(BaseModel):
    ...
    def convert_to_raw(self, data_dict: dict) -> dict:
        return {item_key: self.convert_to_raw_value(item_value) for (item_key, item_value) in data_dict.items()}

    def convert_to_raw_value(self, item_value: Any):
        match item_value:
            case Decimal():
                item_value_dict = {"N": str(item_value)}
            case dict():
                item_value_dict = {"M": json.dumps(item_value)}
            case list():
                item_value_dict = {"L": json.dumps(item_value)}
            case str():
    ...
    def dynamodb_dump(self, exclude_unset: bool = True, include: set[str] | None = None, raw: bool = False):
        ...
        if raw:
            converted = self.convert_to_raw(converted)
        return converted
