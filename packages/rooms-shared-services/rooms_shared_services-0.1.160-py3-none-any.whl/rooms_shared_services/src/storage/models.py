...
class BaseDynamodbModel(BaseModel):
    ...
    def convert_to_raw_value(self, item_value: Any, root_level: bool = False):
        match item_value:
            ...
            case dict():
                item_value_dict = {"M": json.dumps(self.convert_to_raw(item_value))}
            case list():
                item_value_dict = {"L": json.dumps([self.convert_to_raw_value(item_elem) for item_elem in item_value])}
            case str():
