...
class TestSeed(BaseTest):
    ...
    def test_add_seed(self):
        ...
        assert response['status'] == Status.ACTIVE.value, "Response does not have correct status"
    ...
    def test_freeze_seed(self):
        response = self.chariot.update('seed', dict(key=f'#seed#{self.seed}', status=Status.FROZEN.value))[0]
        assert response['status'] == Status.FROZEN.value, "Response does not have correct status"
