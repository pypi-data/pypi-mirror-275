...
use crate::data::runners::load_image_mask;

/// Perform batch morphological profiling from image-mask pairs
///
/// # Arguments
///
/// * `image_path` - Path to the directory containing images
/// * `masks_path` - Path to the directory containing segmentation masks
/// * `image_suffix` - The suffix denoting image files (e.g. "_image")
/// * `masks_suffix` - The suffix denoting segmentation mask files (e.g. "_mask")
...
/// * `pad` - Optional padding to add around cropped objects and bounding masks
...
pub fn profile_image_mask_runner(
    ...
    let (image_suffix, masks_suffix) = match (image_suffix, masks_suffix) {
        (Some(image), Some(masks)) => (image, masks),
        (Some(image), None) => (image, "".to_string()),
        (None, Some(masks)) => ("".to_string(), masks),
    ...
    let masks = std::fs::read_dir(masks_path).expect("Failed to read masks directory")
        .map(|entry| entry.unwrap().path())
        .filter(|path| {
    ...
    progress_log(&format!(
        "Detected and validated {} image-masks pairs.",
    ...
    match images.len().cmp(&image_mask_pairs.len()) {
        std::cmp::Ordering::Less => {
            progress_log(&format!(
                "Warning: {} images were did not have a paired segmentation masks.",
    ...
    let pb = progress_bar(image_mask_pairs.len(), "Computing measurements");
