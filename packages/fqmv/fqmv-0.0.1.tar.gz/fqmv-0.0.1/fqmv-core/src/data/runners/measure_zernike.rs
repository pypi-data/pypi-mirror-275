...
use crate::descriptors::names::ZERNIKE_DESCRIPTOR_NAMES;

/// Compute zernike descriptors from object images
///
/// # Arguments
///
...
/// # Examples
/// ```no_run
/// use fqmv_core::data::runners::measure_zernike_runner;
///
/// measure_zernike_runner(
...
pub fn measure_zernike_runner(
    ...
    progress_log("Measuring zernike descriptors");
    ...
    let n_objects = if objects_path.is_dir() {
        ...
        let pb = progress_bar(files.len(), "Computing zernike descriptors");
        ...
        let descriptors: Vec<Vec<f64>> = files
            ...
            .map(|file| {
                ...
                let descriptor = zernike_descriptors(&object.to_luma8(), center);
        ...
        write_table_descriptors(
            &descriptors,
            &ZERNIKE_DESCRIPTOR_NAMES.iter().map(|&s| s).collect(),
            identifiers.as_ref(),
            Some(filenames),
        ...
        let descriptors = zernike_descriptors(&object.to_luma8(), center);
    ...
    progress_log(format!(
        "Run complete. Measured zernike moments for {} objects.",
