...
use crate::io::{read_image, read_boxes};
...
/// * `boxes_path` - A string slice that holds the path to the boxes file
///
/// # Examples
///
/// ```no_run
/// use fqmv_core::data::runners::load_image_boxes;
/// let (image, boxes) = load_image_boxes("image.png", "boxes.json").unwrap();
/// ```
pub fn load_image_boxes(
    image_path: &str,
    boxes_path: &str,
) -> Result<(ImageContainer, Vec<Vec<f64>>), Error> {
    let image = read_image(image_path)?;
    let boxes = read_boxes(boxes_path)?;
...
/// Perform batch collection of morphological readouts from image-boxes pairs
///
/// # Arguments
///
...
/// * `output_array_format` - Format to save output boxes and labels (default: "npy")
/// * `output_structure` - Output structure (default: 0); can be 0 (by image) or 1 (by object)
pub fn process_image_boxes_runner(
    ...
    let pb = progress_bar(image_boxes_pairs.len(), "Generating outputs");
