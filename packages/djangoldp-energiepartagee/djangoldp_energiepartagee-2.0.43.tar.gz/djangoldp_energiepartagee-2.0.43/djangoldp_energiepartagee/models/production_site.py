...
class ProductionSite(Model):
    ...
    def technologies_used(self):
        return list(self.energy_productions.filter(technology_used__isnull=False).values_list('technology_used', flat=True).distinct()) if self.visible else None

    @property
    def energies_produced(self):
        return list(self.energy_productions.filter(energy_type__name__isnull=False).values_list('energy_type__name', flat=True).distinct()) if self.visible else None
