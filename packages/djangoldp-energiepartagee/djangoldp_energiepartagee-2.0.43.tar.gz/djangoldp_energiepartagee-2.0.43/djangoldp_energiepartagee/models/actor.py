...
class Actor(Model):
    ...
    def technologies_used(self):
        return list(self.founded_projects.filter(visible=True, production_sites__energy_productions__technology_used__isnull=False).values_list('production_sites__energy_productions__technology_used', flat=True).distinct())

    @property
    def energies_produced(self):
        return list(self.founded_projects.filter(visible=True, production_sites__energy_productions__energy_type__name__isnull=False).values_list('production_sites__energy_productions__energy_type__name', flat=True).distinct())

    @property
    def progress_status(self):
        return list(self.founded_projects.filter(visible=True, production_sites__progress_status__isnull=False).values_list('production_sites__progress_status', flat=True).distinct())
