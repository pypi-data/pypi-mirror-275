...
class CitizenProject(Model):
    ...
    def technology_used(self):
        ...
        if technologies_used and len(technologies_used) > 1:
            ...
            return technologies_used[0] if technologies_used else "unknown"
    ...
    def energy_type(self):
        ...
        if energy_produced and len(energy_produced) > 1:
            return "multi"
        return energy_produced[0] if energy_produced else "unknown"

    @property
    def technologies_used(self):
        return list(self.production_sites.filter(visible=True, energy_productions__technology_used__isnull=False).values_list('energy_productions__technology_used', flat=True).distinct())

    @property
    def energies_produced(self):
        return list(self.production_sites.filter(visible=True, energy_productions__energy_type__name__isnull=False).values_list('energy_productions__energy_type__name', flat=True).distinct())

    @property
    def progress_status(self):
        return list(self.production_sites.filter(visible=True, progress_status__isnull=False).values_list('progress_status', flat=True).distinct())

    @property
    def expected_commissionning_year(self):
        return self.production_sites.filter(visible=True).aggregate(earliest_year=models.Min('expected_commissionning_year'))['earliest_year'] or None

    @property
    def effective_commissionning_year(self):
        return self.production_sites.filter(visible=True, progress_status__iexact='exploitation').aggregate(earliest_year=models.Min('effective_commissionning_year'))['earliest_year'] or None

    @property
    def installed_capacity(self):
        return self.production_sites.filter(visible=True).aggregate(total=models.Sum('energy_productions__installed_capacity'))['total'] or 0

    @property
    def estimated_yearly_production(self):
        return self.production_sites.filter(visible=True).aggregate(total=models.Sum('energy_productions__estimated_yearly_production'))['total'] or 0

    @property
    def estimated_injection_capacity(self):
        return self.production_sites.filter(visible=True).aggregate(total=models.Sum('energy_productions__estimated_injection_capacity'))['total'] or 0

    @property
    def consumption_equivalence(self):
        return self.production_sites.filter(visible=True).aggregate(total=models.Sum('energy_productions__consumption_equivalence'))['total'] or 0

    @property
    def total_investment_budget(self):
        return self.production_sites.filter(visible=True).aggregate(total=models.Sum('total_investment_budget'))['total'] or 0
