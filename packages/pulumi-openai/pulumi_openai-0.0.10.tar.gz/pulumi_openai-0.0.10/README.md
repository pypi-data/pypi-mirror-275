...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `openai` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `Tonkean` with the `ORG` of your provider.
   - replace all instances of the `github.com/tonkean/pulumi-openai` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-openai
  ...
  ignore: []
  ldflags:
  - -X github.com/tonkean/pulumi-openai/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-openai/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-openai_darwin_amd64_v1
│   └── pulumi-resource-openai
├── pulumi-openai_darwin_arm64
│   └── pulumi-resource-openai
├── pulumi-openai_linux_amd64_v1
│   └── pulumi-resource-openai
├── pulumi-openai_linux_arm64
│   └── pulumi-resource-openai
├── pulumi-openai_windows_amd64_v1
│   └── pulumi-resource-openai.exe
└── pulumi-openai_windows_arm64
    └── pulumi-resource-openai.exe
```
