def main():
    ...
    parser.add_argument('--CONSTANT', type=float, default=1, help="constant multiplier for alpha and beta in beta-binomila distribution. Default: 1")
    ...
    kwargs["CONSTANT"] = float ( args.CONSTANT )
    ...
    if kwargs["MODE"] in ["Soft", "Both"]:
        print ("\n\n\n======================================== STEP #4.   EM SOFT  ========================================\n")
        print("\tcheckall(strict) : beneficial if satisifed\tcheckall(lenient) : If doesn't satisfy, terminate the iteration" )
        print("\tFor {} steps, normalization by force. After that, no compulsive normalization".format ( kwargs["COMPULSORY_NORMALIZATION"]  ) )
        ...
        for NUM_CLONE in range(kwargs["NUM_CLONE_TRIAL_START"], kwargs["NUM_CLONE_TRIAL_END"] + 1):
            ...
            if cluster_hard.likelihood_record[ NUM_CLONE ] !=  float("-inf"):
                ...
                for step_index in range(1, kwargs["STEP_NO"]):   # Start from 1-index
                    ...
                    if ( len ( set (step_soft.membership) ) < kwargs["NUM_CLONE_ITER"] ) :
                        print ("\t\t\t\t→ Termination due to empty clone")
                        break
                    step_soft = Mstep.main(input_containpos, df, np_vaf, np_BQ, step_soft, "Soft", **kwargs)     # Finding new mixture (centorid)
                    print("\t\t\tCLEMENT_bm.py : makeone_index : {}\tfp_index : {}\ttn_index : {}".format( step_soft.makeone_index, step_soft.fp_index, step_soft.tn_index  ))

                    if step_soft.makeone_index == []:   # if failed
                        print ("\t\t\t\t→ Termination due to checkall_lenient == False\t{}".format( "\t".join(str(np.round(row, 2)) for row in step_soft.mixture ) ))
                        break
                    if ( miscellaneous.iszerocolumn (step_soft, **kwargs) == True) :
                        print ("\t\t\t\t→ Termination due to empty mixture(centroid)\t{}".format(step_soft.mixture))
                        break
                    if ( len ( set (step_soft.membership) ) < kwargs["NUM_CLONE_ITER"] ) :
                        print ("\t\t\t\t→ Termination due to empty clone")
                        break
                ...
                i = step_soft.max_step_index = step_soft.find_max_likelihood_strictfirst_fromtheend ( 1, kwargs["STEP"] - 1 ) [0]    # Explore the best step from the end


                # If couldn't find the answer from the soft clustering
                if (kwargs["STEP"] == 1):
                    print ("\n\tTerminating this clone iteration because undurable from the 1st soft step")
                elif  (step_soft.likelihood_record [i]  <= -9999999) :
                    print ("\n\tTerminating this clone iteration because highly unlikely in all steps (-9999999)")
                else:  # Most of the  cases. Found the possible answer in the soft clustering
                    if kwargs["VERBOSE"] >= 1:
            ...
            else:   # If couldn't find the answer from the hard clustering
                print ("\tTerminating this clone iteration because any hard clustering is feasible")
    ...
    NUM_CLONE_hard , NUM_CLONE_soft = [], []    # Clone number order in hard clustering and soft clustering, respectively.
    ...
    if kwargs["MODE"] in ["Hard", "Both"]:
        for i, priority in enumerate(["1st"]):   # "2nd"
            ...
            if cluster_soft.mixture_record [ NUM_CLONE_hard[i] ] == []:       # If soft clusterring failed in the best hard clustering number
                ...
                if (priority == "1st") & (kwargs["MODE"] in ["Both"]):
                    ...
                    if (kwargs["NUM_BLOCK"] >= 2) & ("soft" in DECISION) :     # When more than 2D, final decision sholud based on NUM_CLONE_hard
            ...
            if kwargs["SCORING"] == False:
            ...
            elif kwargs["SCORING"] == True:     
                # Find mixture (centroid) of False variants (FV)
                ...
                # Count how many rescue data are
                rescue_data = []
                for k in range (kwargs["NUM_MUTATION"]):
            ...
            if ISPARENT == True:
                ...
                f = io.StringIO()
                with contextlib.redirect_stdout(f):
                    membership_child = set ( cluster_hard.makeone_index_record[ NUM_CLONE_hard[i] ] )  
                    membership_outside = set (range (0, NUM_CLONE_hard [i] )) - membership_child - set ( [cluster_hard.fp_index_record[ NUM_CLONE_hard[i] ] ] )   
    ...
    if kwargs["MODE"] in ["Soft", "Both"]:
        ...
        if DECISION == "soft(hard)_1st":
            ...
            if ( cluster_soft.mixture_record [NUM_CLONE_hard[0]] == [] ):
                ...
                elif kwargs["SCORING"] == True:  # Scoring and performance measurement of Soft clustering
                    # Find mixture (centroid) of False variants (FV)
                ...
                if ISPARENT == True:
                    ...
                    f = io.StringIO()
                    with contextlib.redirect_stdout(f):
                        membership_child = set ( cluster_soft.makeone_index_record[ NUM_CLONE_CLEMENT ] )            
                        membership_outside = set (range (0, NUM_CLONE_CLEMENT )) - membership_child - set ( [cluster_soft.fp_index_record[NUM_CLONE_CLEMENT ] ] ) 
        ...
        if DECISION == "soft_1st":
            ...
            for i, priority in enumerate(["1st"]):        
                ...
                if ISPARENT == True:
                    ...
                    f = io.StringIO()
                    with contextlib.redirect_stdout(f):
                        membership_child = set ( cluster_soft.makeone_index_record[ NUM_CLONE_soft[i] ] )            
                        membership_outside = set (range (0, NUM_CLONE_soft [i] )) - membership_child - set ( [cluster_soft.fp_index_record[ NUM_CLONE_soft[i] ] ] )  
    ...
    if kwargs["MODE"] in ["Soft", "Both"]:
        ...
        if  ( NUM_CLONE_soft == []):     # If no possible answer in Soft clustering
            DECISION = "hard_1st"
            soft_std = float("inf")
            print ( "DECISION\t{}\nhard_mixture\t{}\nsoft_mixture\tNone\nHard clustering selection because no possible soft clustering results".format( DECISION,  "\t".join(str(np.round(row, 2)) for row in cluster_hard.mixture_record [NUM_CLONE_hard[0]] )    ))
            with open ( kwargs["CLEMENT_DIR"]+ "/CLEMENT_decision.evidence.txt"  , "w", encoding = "utf8") as output_file:
                print ( "DECISION\t{}\nhard_mixture\t{}\nsoft_mixture\tNone\nHard clustering selection because no possible soft clustering results".format( DECISION, "\t".join(str(np.round(row, 2)) for row in cluster_hard.mixture_record [NUM_CLONE_hard[0]] )  ), file = output_file)


        elif (cluster_soft.mixture_record [ NUM_CLONE_hard[0] ] == []):  # Selected the best clone number based on soft clustering, because best clone number based on hard clustering is unavailable in soft clustering
            if (kwargs ["MAKEONE_STRICT"] == 3) & (cluster_soft.mixture_record [ NUM_CLONE_soft[0] ] != []) & (cluster_soft.checkall_strict_record  [ NUM_CLONE_soft[0]  ] == True ) :  # Special cases such as BioData
                DECISION = "soft_1st"
                hard_std, soft_std = float("inf"), float("inf")
                moved_col_list, moved_col_len = [], 0
                print ( "DECISION\t{}\nhard_mixture\t{}\nsoft_mixture\t{}\nSelected the best clone number based on soft clustering, because best clone number based on hard clustering is unavailable in soft clustering".format( DECISION,  "\t".join(str(np.round(row, 2)) for row in cluster_hard.mixture_record [NUM_CLONE_hard[0]] ), "\t".join(str(np.round(row, 2)) for row in cluster_soft.mixture_record [NUM_CLONE_soft[0]] )    ))
                with open ( kwargs["CLEMENT_DIR"]+ "/CLEMENT_decision.evidence.txt"  , "w", encoding = "utf8") as output_file:
                    print ( "DECISION\t{}\nhard_mixture\t{}\nsoft_mixture\t{}\nSelected the best clone number based on soft clustering, because best clone number based on hard clustering is unavailable in soft clustering".format( DECISION,  "\t".join(str(np.round(row, 2)) for row in cluster_hard.mixture_record [NUM_CLONE_hard[0]] ), "\t".join(str(np.round(row, 2)) for row in cluster_soft.mixture_record [NUM_CLONE_soft[0]]) ) , file = output_file)
            else: # Utilize hard clustering result
                DECISION = "hard_1st"
                hard_std, soft_std = float("inf"), float("inf")
                print ( "DECISION\t{}\nhard_mixture\t{}\nsoft_mixture\tNone\nHard clustering selection because no possible soft clustering results".format( DECISION,  "\t".join(str(np.round(row, 2)) for row in cluster_hard.mixture_record [NUM_CLONE_hard[0]] )    ))
                with open ( kwargs["CLEMENT_DIR"]+ "/CLEMENT_decision.evidence.txt"  , "w", encoding = "utf8") as output_file:
