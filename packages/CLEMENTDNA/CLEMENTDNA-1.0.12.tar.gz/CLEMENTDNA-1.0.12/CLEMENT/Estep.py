...
def expected_calculator (  i, j, k, mixture, df, input_containpos, **kwargs ):
    ...
    a, b = alt_calc * kwargs["CONSTANT"], (depth_obs - alt_calc) * kwargs["CONSTANT"]              # alt_expected, ref_expected
...
def calc_posterior(input_containpos, df,  np_vaf, np_BQ, step, k, **kwargs):
    ...
    posterior_allsample = np.zeros(kwargs["NUM_CLONE"], dtype="float64")           
    likelihood_allsample = np.zeros(kwargs["NUM_CLONE"], dtype="float64")          
    ...
    proportion = np.unique(step.membership, return_counts = True)[1] / kwargs["NUM_MUTATION"]   # number of variants per clone


    # Denominator calculation at once
    ...
    for i in range(kwargs["NUM_BLOCK"]):
        for j in range(kwargs["NUM_CLONE"]): 
            ...
            try:
                if (alt_obs == 0) & (mixture [i][j] == 0):    # TN
                    ...
                    L_denominator[i] += (equivalent * scipy.stats.binom.pmf(n = depth_obs, p = SEQ_ERROR, k = alt_obs))          
                else:   # FN or TN
        ...
        if L_denominator[i] <= 0 :  # Doesn't make sense. But, don't know why.
    ...
    for j in range(kwargs["NUM_CLONE"]): 
        ...
        for i in range(kwargs["NUM_BLOCK"]):
            ...
            if mixture[i][j] == 0:    # TN or FP   (Binomial)
                ...
                try:
                    ...
                    if(kwargs ["DEBUG"] == True ) :
                        if  ( k in debug_k)  :            
                            np.set_printoptions(suppress=True)   # Suppress Scientific expression
                            if  kwargs["MODEL"] == "betabinomial":  
            ...
            # Plus, not multiplying, because it's log.
        ...
        elif posterior_allsample[j] == max_posterior_allsample :     # If the scores are equal
            max_posterior_clone_candidate.append (j)
    
    try:
        max_clone = random.choice( max_posterior_clone_candidate )
    except:
        print ( "Error: max clone choice is abnormal", k, max_posterior_clone_candidate, posterior_allsample )
    ...
    if kwargs["OPTION"] in ["Hard", "hard"]:
        ...
        return list(posterior_allsample), soft_posterior_allsample, soft_likelihood_allsample, posterior_allsample_normalized, likelihood_allsample_normalized, soft_posterior_allsample_normalized, soft_likelihood_allsample_normalized,  max_clone      # Likelihood: determining termination condition,  Posterior : when finding  mixture
...
def main (input_containpos, df, np_vaf, np_BQ, step, **kwargs):
    ...
    if  step.fp_index in set(step.membership):   # At lease one variant (FP) is needed to set step.includefp == Ture
        step.includefp = True
