...
class Nlp(HasObjective[SymType], SupportsDeepcopyAndPickle):
    ...
    def to_opti(self) -> cs.Opti:
        # choose solver
        opti = cs.Opti(
            "conic"
            if hasattr(self, self._solver_type) and cs.has_conic(self._solver_type)
            else "nlp"
        )

        # create same variables and parameters
        vars = {n: opti.variable(*v.shape) for n, v in self._vars.items()}
        pars = {n: opti.parameter(*p.shape) for n, p in self._pars.items()}

        # create same constraints

        # create same objective

        # set same solver options, if any
