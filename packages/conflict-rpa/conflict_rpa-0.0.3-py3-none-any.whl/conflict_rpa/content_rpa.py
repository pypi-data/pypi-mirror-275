...
{   
    ...
    'bash_operation_include': bool // 文本信息中是否有包含一系列可执行的bash命令，这些命令在顺序执行后可以完成 应用安装/执行/初始化等操作
    ...
    'scripts': List[str] //if bash_operation_include=True，那么请按顺序，严格抽取bash命令（尤其是关于installation，getting start等内容）(不包括其他任何语言的代码块！要在bash中执行)并组织为列表，这样的列表在顺序执行后可以完成网站中项目的基础功能。如果文本中包含多种执行方法，则选择其中最匹配当前执行环境的方法.果命令中包含类似`/where/you/keep/code`的路径，将这些自定义路径修改为当前目录即可
}''' + f'''
当前程序运行的环境信息为
```
{get_running_env()}
...
def content_rpa(content: str):
    analysis = analyse_tutorial(content)
    if analysis['bash_operation_include']:
