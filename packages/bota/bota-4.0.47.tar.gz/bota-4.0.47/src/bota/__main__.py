...
def create_cluster(cluster_name, machine_type, machines):
    """Create the cluster"""
    if machines <= 0:
    ...
    click.echo(f"    - {machines} machines running of type {machine_type}")
    click.echo(f"    - A static IP address")
    click.echo("------------")

    ip_address = run_create_cluster_commands(cluster_name, machines, machine_type)
