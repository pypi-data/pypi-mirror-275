...
if "import":
    ...
    import os
    import onnxruntime
    from utilmy import json_load, json_save, log


###################################################################################################
def test_onnx(dirmodel_onnx: str, query_str: str):
    tokenizer       = AutoTokenizer.from_pretrained(dirmodel_onnx)
    id2label        = json_load( dirmodel_onnx + "/classes_index.json")
    dirmodel_onnx   = dirmodel_onnx + '/model.onnx' 
...
def test_create_model(path_of_the_model: str='tuenguyen/token_classification_model'):
    model = AutoModelForTokenClassification.from_pretrained(path_of_the_model)
    log(model)
