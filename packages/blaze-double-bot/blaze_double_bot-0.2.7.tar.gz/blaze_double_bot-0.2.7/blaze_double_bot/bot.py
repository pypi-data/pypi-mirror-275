...
class BlazeDoubleBot:
    ...
    def get_history(self, size):
        try:
            ...
            print(self.get_text('Error getting history:'), e)
            return ['n'] * size
...
{self.get_text("Completed")} {datetime.now().strftime("%m/%d/%Y %H:%M")}
{self.get_text("Waiting for result ...")}
''')

    def print_bet_result(self, history, color):
        print(f"{self.get_text('Win ->')} {history}" if history[-1] == self.color_dict[color] else f"{self.get_text('Loss ->')} {history}")
            ...
            while True:
                if self.stop_loss <= self.current_balance <= self.stop_win and self.current_balance - self.bet_amount >= 0:
                    if self.wait_for_next_round():
                        ...
                        if color:
                            print(self.get_text('Betting strategy -> '),history)
                            print()
