...
project: default
domain: development
base_image: ghcr.io/thomasjpfan/workspace:0.0.1
...
class WorkspaceConfig(DataClassYAMLMixin, DataClassJSONMixin):
    ...
    image_spec: Optional[ImageSpecConfig] = None
    resources: Optional[ResourceConfig] = None
    on_startup: Optional[str] = None

    def __post_init__(self):
        if self.image_spec and os.getenv("UNIONAI_DEV"):
            ...
            self.image_spec = ImageSpecConfig(
                name="workspace",
                builder="unionai",
        ...
        if self.base_image is None and self.image_spec is None:
            raise ValueError("base_image or image_spec must be configure.")
...
class WorkspaceResolver(TaskResolverMixin):
    ...
    def get_task_for_workspace(
        ...
        return PythonFunctionTask(
            ...
            task_type="workspace",
            task_resolver=self,
            requests=config.resources if config else None,
            container_image=config.image_spec if config else None,
        )
