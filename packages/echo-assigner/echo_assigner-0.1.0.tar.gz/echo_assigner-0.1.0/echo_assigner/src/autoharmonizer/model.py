...
def create_training_data(segment_length=SEGMENT_LENGTH, 
                         chord_types_path=os.path.dirname(__file__)+"/bin/chord_types.bin", 
                         corpus_path=os.path.dirname(__file__)+"/bin/data_corpus.bin",  
                         val_ratio=VAL_RATIO):
    ...
    for songs_idx in range(len(data_corpus)):
...
def build_model(segment_length, rnn_size, num_layers, dropout, 
                weights_path=os.path.dirname(__file__)+"/bin/weights.hdf5", 
                chord_types_path=os.path.dirname(__file__)+"/bin/chord_types.bin", 
                training=True):
