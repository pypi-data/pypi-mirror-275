...
class EntityItemDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def validate(self):
        exist = self.tree.get_item(self.editor.text())
...
class EntityItem(QtWidgets.QTreeWidgetItem):
    ...
    def refresh(self):          
        ...
        elif self.editable and 'Create' not in self.display_name:
...
class EntityList(QtWidgets.QTreeWidget):
    ...
    def get_item(self, display_name=None, name=None):
        if display_name:
            match_index = [i for i in range(self.topLevelItemCount()) if self.topLevelItem(i).display_name == display_name]
        else:
            match_index = [i for i in range(self.topLevelItemCount()) if self.topLevelItem(i).name == name]
    ...
    def on_item_changed(self, item, column):
        if item.flags() & QtCore.Qt.ItemIsEditable:
            ...
            if self.header_text == 'Tasks' and self.page_widget.lowercase_for_task() is True:
...
class TargetWizardDialog(QtWidgets.QDialog):
    ...
    def __init__(self, files_list, item):
        ...
        self.asset_flow = ['Asset Types', 'Assets', 'Tasks', 'Files']
    ...
    def build_flow(self, entity_type):
        # Clear current flow
        ...
        for i, entity_display_name in enumerate(flow):
            ...
            for label, goto_oid in self.split_oid:
                ...
                map_type = label.split(':')[0].replace('_', ' ')
