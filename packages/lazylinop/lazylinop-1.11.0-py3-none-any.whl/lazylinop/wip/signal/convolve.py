...
def _oaconvolve(in1s: int, in2: np.ndarray, mode: str = 'full',
    ...
    if in1s > (2 * K):
        ...
        D = diag(fft(B, norm=None, workers=workers)
                 @ eye(B, n=K, k=0) @ in2, k=0)
        F = fft(B, norm='1/n', workers=workers).H @ D \
            @ fft(B, norm=None, workers=workers)
        ...
        C = fft(Y, norm='1/n', workers=workers).H \
            @ D @ F @ eye(Y, n=in1s, k=0)
