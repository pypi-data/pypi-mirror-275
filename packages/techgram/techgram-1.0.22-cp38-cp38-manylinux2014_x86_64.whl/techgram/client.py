...
from techgram.types import Identifier, Listener, User, TermsOfService
...
class Client(Methods):
                ...
                if isinstance(r, raw.types.upload.File):
                    while True:
                        ...
                        if progress:
                            ...
                            if inspect.inspect.iscoroutinefunction(progress):
                                await func()
                            else:
    ...
    async def listen(
        self,
        filters: Optional[techgram.filters.Filter] = None,
        ...
        try:
            ...
            if callable(utils.config.timeout_handler):
                if inspect.iscoroutinefunction(utils.config.timeout_handler.__call__):
                    await utils.config.timeout_handler(pattern, listener, timeout)
                else:
                    await self.loop.run_in_executor(
                        None, utils.config.timeout_handler, pattern, listener, timeout
                    )
            elif utils.config.throw_exceptions:
    ...
    async def stop_listener(self, listener: Listener):
        ...
        if callable(utils.config.stopped_handler):
            if inspect.iscoroutinefunction(utils.config.stopped_handler.__call__):
                await utils.config.stopped_handler(None, listener)
            else:
                await self.loop.run_in_executor(
                    None, utils.config.stopped_handler, None, listener
                )
        elif utils.config.throw_exceptions:
