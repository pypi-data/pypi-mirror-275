...
class Cluster:
    ...
    def find(cls, aws_session, names):
        ...
        for head_node in ec2.instances.filter(Filters=headnodes_filters):
            if cls.get_cluster_name(head_node) == cls.get_cluster_name(head_node.vpc):  # already listed above
                log.debug("Skipping head node %r as already listed for VPC %r",
                          cls.get_resource_name(head_node), cls.get_resource_name(head_node.vpc))
                continue
            cluster_name = cls.get_cluster_name(head_node)
    ...
    @classmethod
    def get_cluster_name(cls, resource: ResourceT) -> str:
        cluster_name = next(
            (
                cluster_name
                for cluster_name_tag in BCM_CLUSTER_NAME_TAGS
        ...
        if not cluster_name:
            raise CODException(f"No cluster name tag found in {cls.get_resource_name(resource)}")
        return cluster_name
