...
class Concatenate(Layer):
    def __init__(self, idx:int, size:int, axis:int, input_shapes:list, output_shape:list, activation_function:ActivationFunctions):
        ...
        if any((type(shape) != int for shape in input_shape) for input_shape in self.input_shapes):
        ...
        if self.axis == 1 and self.output_channels != sum(self.input_shapes[:][1]):
        ...
        if self.axis == 1 and self.output_channels != sum(self.input_shapes[:][2]):
        ...
        if self.axis == 1 and self.output_channels != sum(self.input_shapes[:][3]):
