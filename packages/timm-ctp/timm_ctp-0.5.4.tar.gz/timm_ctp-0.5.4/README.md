...
* Add ConvNeXT models /w weights from official impl (https://github.com/facebookresearch/ConvNeXt), a few perf tweaks, compatible with timm_ctp features
...
### Aug 18, 2021
* Optimizer bonanza!
  * Add LAMB and LARS optimizers, incl trust ratio clipping options. Tweaked to work properly in PyTorch XLA (tested on TPUs w/ `timm_ctp bits` [branch](https://github.com/rwightman/pytorch-image-models/tree/bits_and_tpu/timm_ctp/bits))
...
### April 1, 2021
* Add snazzy `benchmark.py` script for bulk `timm_ctp` model benchmarking of train and/or inference
* Add Pooling-based Vision Transformer (PiT) models (from https://github.com/naver-ai/pit)
  ...
  * Some `timm_ctp` cleanup/style tweaks and weights have hub download support
...
* Integrate [Hugging Face model hub](https://huggingface.co/models) into timm_ctp create_model and default_cfg handling for pretrained weight and config sharing (more on this soon!)
...
Py**T**orch **Im**age **M**odels (`timm_ctp`) is a collection of image models, layers, utilities, optimizers, schedulers, data-loaders / augmentations, and reference training / validation scripts that aim to pull together a wide variety of SOTA models with ability to reproduce ImageNet training results.
...
My current [documentation](https://rwightman.github.io/pytorch-image-models/) for `timm_ctp` covers the basics.

[timm_ctpdocs](https://fastai.github.io/timm_ctpdocs/) is quickly becoming a much more comprehensive set of documentation for `timm_ctp`. A big thanks to [Aman Arora](https://github.com/amaarora) for his efforts creating timm_ctpdocs.

[paperswithcode](https://paperswithcode.com/lib/timm_ctp) is a good resource for browsing the models within `timm_ctp`.
