...
def create_default_config(config_file):
    ...
    config['OutputOptions'] = {'OutputDirectory': ''}
...
def prompt_config():
    ...
    if filter_by_release.lower() == 'yes' or filter_by_release.lower() == 'y':
        config['ExclusionOptions'] = {
        ...
        config['ExclusionOptions']['IncludeCompilation'] = 'False' if exclude_compilation.lower() == 'yes' or exclude_compilation.lower() == 'y' else 'True'
        
        exclude_live = input("Exclude live recordings? (yes/no): ")
        config['ExclusionOptions']['IncludeLive'] = 'False' if exclude_live.lower() == 'yes' or exclude_live.lower() == 'y' else 'True'
        
        exclude_broadcast = input("Exclude broadcasts? (yes/no): ")
        config['ExclusionOptions']['IncludeBroadcast'] = 'False' if exclude_broadcast.lower() == 'yes' or exclude_broadcast.lower() == 'y' else 'True'
        
        exclude_single = input("Exclude singles? (yes/no): ")
        config['ExclusionOptions']['IncludeSingle'] = 'False' if exclude_single.lower() == 'yes' or exclude_single.lower() == 'y' else 'True'
        
        exclude_soundtrack = input("Exclude soundtracks? (yes/no): ")
        config['ExclusionOptions']['IncludeSoundtrack'] = 'False' if exclude_soundtrack.lower() == 'yes' or exclude_soundtrack.lower() == 'y' else 'True'
        
        exclude_demo = input("Exclude demo versions? (yes/no): ")
        config['ExclusionOptions']['IncludeDemo'] = 'False' if exclude_demo.lower() == 'yes' or exclude_demo.lower() == 'y' else 'True'
        
        exclude_ep = input("Exclude EPs? (yes/no): ")
        config['ExclusionOptions']['IncludeEP'] = 'False' if exclude_ep.lower() == 'yes' or exclude_ep.lower() == 'y' else 'True'

    use_spotify = input("Do you want to fetch data from Spotify URLs? (yes/no): ")
    if use_spotify.lower() == 'yes' or use_spotify.lower() == 'y':
...
def clean_track_name(track_name):
    ...
    track_name = re.sub(r'\[[^\]]*\]', '', track_name)
    # Remove " - YYYY remaster" and anything following it
    track_name = re.sub(r' - \d{4} remaster.*$', '', track_name)
    # Remove " - ____" at the end
    track_name = re.sub(r' - [^\-]*$', '', track_name)
...
def convert_to_xml(csv_file, config):
    ...
    with open(csv_file, 'r', encoding='utf-8-sig') as file:
        ...
        for row in reader:
            ...
            # Apply custom exclusion conditions based on config
...
def get_playlist_tracks(playlist_id, config):
    ...
    for item in all_tracks:
        track = item['track']
        track_details = {
            'Playlist Name': playlist_name,  # Add playlist name to track details
...
def main():
    ...
    parser.add_argument('input_file', nargs='?', help='Path to the input CSV file or Spotify playlist URL')
    ...
    if not os.path.exists(args.config):
        ...
        if create_config.lower() == 'yes' or create_config.lower() == 'y':
    ...
    if args.input_file and args.input_file.startswith("https://open.spotify.com"):
        if not (config.get('SpotifyAPI', 'ClientID') and config.get('SpotifyAPI', 'ClientSecret')):
            ...
            return
        playlist_id = args.input_file.split('/')[-1]
        playlist_tracks_details = get_playlist_tracks(playlist_id, config)
        # Corrected the filename for the CSV file
        csv_output_file = f"{playlist_tracks_details[0]['Playlist Name'].replace(' ', '_')}.csv"
        with open(csv_output_file, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['Playlist Name', 'Track Name', 'Artist Name(s)', 'Album']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(playlist_tracks_details)

        args.input_file = csv_output_file

    if args.input_file:
        convert_to_xml(args.input_file, config)
    else:
        print("Error: Please provide an input file or use the --reconfigure flag.")
