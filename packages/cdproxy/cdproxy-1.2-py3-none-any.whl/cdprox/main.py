...
def main():
    ...
    if args.command == 'add':
        ...
        if not s3_access_key or not s3_secret_key or not s3_region or not s3_bucket:
            print("Error: S3 access key, secret key, region, and bucket name must be set as environment variables.")
            sys.exit(1)
        ...
        local_caddyfile_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), s3_caddy_filename)
