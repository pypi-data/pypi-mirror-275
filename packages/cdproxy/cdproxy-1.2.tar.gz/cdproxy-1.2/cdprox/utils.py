...
def download_caddyfile_from_s3(s3_client, bucket_name, s3_caddy_filename, local_caddyfile_path):
    try:
        ...
        print(f"Downloaded Caddyfile from S3 bucket {bucket_name}")
    except NoCredentialsError:
        print("AWS credentials not available.")
    except PartialCredentialsError:
        print("Incomplete AWS credentials provided.")
    except Exception as e:
        print(f"Error downloading Caddyfile from S3: {e}")
...
def upload_caddyfile_to_s3(s3_client, bucket_name, s3_caddy_filename, local_caddyfile_path):
    try:
        ...
        print(f"Uploaded Caddyfile to S3 bucket {bucket_name}")
    except NoCredentialsError:
        print("AWS credentials not available.")
    except PartialCredentialsError:
        print("Incomplete AWS credentials provided.")
    except Exception as e:
        print(f"Error uploading Caddyfile to S3: {e}")
