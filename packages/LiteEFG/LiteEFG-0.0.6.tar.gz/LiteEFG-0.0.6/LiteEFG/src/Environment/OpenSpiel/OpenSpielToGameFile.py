...
def InfosetName(node, idx):
    infoset = "pl%d_%d__"%(0, idx) + node if isinstance(node, str) \
...
class OpenSpielEnv(LiteEFG.FileEnv):
    ...
    def get_value(self, player: int, node: LiteEFG.GraphNode) -> dict:
    ...
    def interact(self, policy: TabularPolicy, controlled_player=0, reveal_private=True, epochs=1000) -> None:
        ...
        for epoch in range(epochs):
            ...
            if not reveal_private:
                print("The game terminates at: ", s.information_state_string(controlled_player))
            else:
                print(s)
