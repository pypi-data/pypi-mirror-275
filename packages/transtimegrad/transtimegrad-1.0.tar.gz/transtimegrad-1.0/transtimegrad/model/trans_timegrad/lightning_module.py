...
class TransTimeGradLightningModule(LightningModule):
    """
    A ``LightningModule`` class that can be used to train a
    ``TransTimeGradModel`` with PyTorch Lightning.

    This is a thin layer around a (wrapped) ``TransTimeGradModel`` object,
    that exposes the methods to evaluate training and validation loss.

    Parameters
    ----------
    model
        ``TransTimeGradModel`` to be trained.
    lr
    ...
    def __init__(
        ...
        self.model = TransTimeGradModel(**model_kwargs)
        self.lr = lr
