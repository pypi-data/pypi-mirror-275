...
from sentineltoolkit.exceptions import DataSemanticConversionError
from sentineltoolkit.typedefs import (
    FileNameGenerator,
...
PATTERNS: dict[L_DataFileNamePattern, str] = {
...
COMPILED_PATTERNS: dict[str, re.Pattern[str]] = {fmt: re.compile(pattern) for fmt, pattern in PATTERNS.items()}


def detect_filename_pattern(filename: str) -> L_DataFileNamePattern:
    for fmt in PATTERN_ORDER:
        co = COMPILED_PATTERNS[fmt]
