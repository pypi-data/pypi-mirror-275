...
async def display_livekit_frames(livekit_q: asyncio.Queue[livekit.VideoFrame], track_id: str) -> None:
    ...
    argb_frame: livekit.ArgbFrame | None = None
...
def handle_new_track(track_id: str) -> None:
    livekit_queue: asyncio.Queue[livekit.VideoFrame] = asyncio.Queue()
    display_livekit_task = asyncio.create_task(display_livekit_frames(livekit_queue, track_id))
...
async def setup_connection(url: str, flag: Future[bool]) -> None:
    ...
    logging.info(info)
    info["token"] = create_access_token(api_key=info["api_key"], api_secret=info["api_secret"], room_name=info["room"], identity=info["identity"])
    logging.info(info)
    ...
    try:
        ...
        def on_track_subscribed(track: livekit.Track,
                                ...
                                participant: livekit.RemoteParticipant) -> None:
