...
class AMQPProvider(EncodedProvider, AMQPProviderConfig, AMQPUtils):
    ...
    def declare_exchange(self, exchange: Exchange, arguments: dict = {}) -> None:
        generated_arguments = self.generate_props(arguments)
        self.consumer_channel.exchange_declare(exchange.name, exchange.exchange_type.value, durable=exchange.durable,
                                               auto_delete=exchange.auto_delete, arguments=generated_arguments)
    ...
    def publish(self, routing_key: str, message: str, common_id: str = str(uuid.uuid4()),
                ...
                delay: int = 0, logging_enabled: bool = False, exchange_args: dict = {}) -> None:
        try:
            try:
            ...
            self.declare_destination(routing_key, exchange, exchange_args)
    ...
    def declare_destination(self, queue_name: str, exchange: Optional[Exchange], exchange_args: dict) -> None:
        if self.destination_declared:
            return

        if exchange:
            self.declare_exchange(exchange, exchange_args)
        else:
